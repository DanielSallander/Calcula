...
//     - cap.fileImportMedia: Ask you to pick a picture and store it in this workbook (only the one file you pick; it is given a reference to the picture, never the picture's data)
...
//   ui.htmlInput
//     - render.setHitRegions: Claim rectangles of its own HTML frame for clicks; clicks outside them still reach the grid, and Design Mode suspends them all
//   ui.pane
//     - pane.close: Close its own task pane from code, at any point while you work; a form of its own you have placed on a sheet it may NOT close — only you remove that from the sheet
//     - pane.dock: Add a task pane of its own (labels, inputs, choices, buttons) that stays beside the grid while you work, and read what you enter in it; it appears in front only for a few seconds after you did something that ran the script, or allowed it this permission — otherwise it waits in the panel list until you open it — and where you last put a pane of this script is where it comes back; a field it bound to a cell shows that cell and writes it back as soon as you change it, and closing the pane does not undo that (on the sheet you were looking at when it opened, for a restricted script); you can close or move the pane at any time
//     - pane.list: List its OWN open task panes and the forms of its own you have placed on sheets, so it knows which one to change while you work; it learns nothing about any other script's surfaces
//     - pane.reveal: Bring its task pane to the front while you work, but only for a few seconds after you used it or ran the script, and only if it is docked where that is possible; a form of its own you have placed on a sheet is never brought forward — you scroll to it — and it is told when it may not
//     - pane.setBadge: Put a short badge (a count, a word) on its task pane's tab while you work, or clear it
//     - pane.update: Change what its task pane, or a form of its own you have placed on a sheet, shows while you work (values, enabled or hidden fields, choices, a message)
//   ui.shortcut
...
declare interface ScriptFileApi {
   ...
   *
   * Calcula policy (generated): Ask you to pick a picture and store it in this workbook (only the one file you pick; it is given a reference to the picture, never the picture's data).
   * Reach: broker `cap.fileImportMedia`, restricted tier, class file, requires the `file.picker` capability. Limits: maxBytes 8,388,608, maxPixels 40,000,000.
   */
...
declare interface UnlockedAPI {
   ...
   *
   * Calcula policy (generated): Hide (or unhide) a band of rows on the active sheet — the same act as right-click ▸ Hide, in one undo step.
   * Reach: broker `api.setRowsHidden`, unlocked tier, class mutate. Limits: maxSpan 1,048,576.
   */
   ...
   *
   * Calcula policy (generated): Hide (or unhide) a band of columns on the active sheet — the same act as right-click ▸ Hide, in one undo step.
   * Reach: broker `api.setColumnsHidden`, unlocked tier, class mutate. Limits: maxSpan 1,048,576.
   */
   ...
   * ```
   *
   * Calcula policy (generated): List which rows are hidden on a sheet — separately: the ones hidden by hand, and the ones hidden by anything at all (hand, filter, or collapsed group).
   * Reach: broker `api.getHiddenRows`, unlocked tier, class read.
   */
   ...
   *  column filters, so `effective` is by hand OR collapsed outline group.
   *
   * Calcula policy (generated): List which columns are hidden on a sheet — separately: the ones hidden by hand, and the ones hidden by anything at all (hand or collapsed group).
   * Reach: broker `api.getHiddenColumns`, unlocked tier, class read.
   */
   ...
   * ```
   *
   * Calcula policy (generated): Place a picture that is already stored in this workbook onto the sheet currently shown (it can name a picture, never supply one).
   * Reach: broker `api.createPicture`, unlocked tier, class mutate.
   */
   ...
   * ```
   *
   * Calcula policy (generated): Draw a shape (rectangle, arrow, star and so on) on the sheet currently shown — NOT undoable: Ctrl+Z will not remove it.
   * Reach: broker `api.createShape`, unlocked tier, class mutate.
   */
   ...
   *
   * Calcula policy (generated): Delete a shape, button or picture from the sheet currently shown, along with any script attached to it — NOT undoable: Ctrl+Z will not bring it back.
   * Reach: broker `api.deleteShape`, unlocked tier, class mutate.
   */
   ...
   * ```
   *
   * Calcula policy (generated): Create a floating range (a small movable grid of real cells, referenced from formulas as Name!A1) on the sheet currently shown — creation itself is NOT undoable: Ctrl+Z will not remove it.
   * Reach: broker `api.createFloatingRange`, unlocked tier, class mutate.
   */
   ...
   *
   * Calcula policy (generated): Delete a floating range and its cells; formulas referencing it show #REF! — NOT undoable, and like deleting a sheet it ENDS the undo history.
   * Reach: broker `api.deleteFloatingRange`, unlocked tier, class mutate.
   */
   ...
   *
   * Calcula policy (generated): Write values or formulas into a floating range's cells (undoable, recalculates like any cell edit).
   * Reach: broker `api.floatingRangeSetCells`, unlocked tier, class mutate.
   */
   ...
   *
   * Calcula policy (generated): Read a floating range's cell values and formulas.
   * Reach: broker `api.floatingRangeGetCells`, unlocked tier, class read.
   */
   ...
   *
   * Calcula policy (generated): Change how many rows and columns a floating range shows (shrinking hides cells, it never deletes them; undoable).
   * Reach: broker `api.floatingRangeResize`, unlocked tier, class mutate.
   */
...
declare interface PaneControlHandle {
  ...
  readonly value: unknown;
  /**
   * Calcula policy (generated): Change what its task pane, or a form of its own you have placed on a sheet, shows while you work (values, enabled or hidden fields, choices, a message).
   * Reach: broker `pane.update`, restricted tier, class emit, requires the `ui.pane` capability. Limits: perSecond 30.
   */
  set(value: unknown): void;
  /** label / button / progress caption.
   *
...
declare interface ScriptPaneApi {
   ...
   *
   * Calcula policy (generated): Add a task pane of its own (labels, inputs, choices, buttons) that stays beside the grid while you work, and read what you enter in it; it appears in front only for a few seconds after you did something that ran the script, or allowed it this permission — otherwise it waits in the panel list until you open it — and where you last put a pane of this script is where it comes back; a field it bound to a cell shows that cell and writes it back as soon as you change it, and closing the pane does not undo that (on the sheet you were looking at when it opened, for a restricted script); you can close or move the pane at any time.
   * Reach: broker `pane.dock`, restricted tier, class ui, requires the `ui.pane` capability. Limits: maxNodes 200, maxDepth 8, maxInputs 64, maxPanes 3, perMinute 10.
   */
   ...
   *
   * Calcula policy (generated): Bring its task pane to the front while you work, but only for a few seconds after you used it or ran the script, and only if it is docked where that is possible; a form of its own you have placed on a sheet is never brought forward — you scroll to it — and it is told when it may not.
   * Reach: broker `pane.reveal`, restricted tier, class ui, requires the `ui.pane` capability. Limits: perMinute 6.
   */
  reveal(): Promise<PaneRevealResult>;
  /** A short badge (a count, a word; max 8 characters) on the pane's tab; null clears it.
   *
   * Calcula policy (generated): Put a short badge (a count, a word) on its task pane's tab while you work, or clear it.
   * Reach: broker `pane.setBadge`, restricted tier, class emit, requires the `ui.pane` capability. Limits: maxChars 8.
   */
   ...
   *
   * Calcula policy (generated): Close its own task pane from code, at any point while you work; a form of its own you have placed on a sheet it may NOT close — only you remove that from the sheet.
   * Reach: broker `pane.close`, restricted tier, class emit, requires the `ui.pane` capability.
   */
  close(): void;
  /**
   ...
   *
   * Calcula policy (generated): List its OWN open task panes and the forms of its own you have placed on sheets, so it knows which one to change while you work; it learns nothing about any other script's surfaces.
   * Reach: broker `pane.list`, restricted tier, class read, requires the `ui.pane` capability.
   */
...
declare interface ShapeContext extends BaseObjectContext {
  ...
  render: {
     ...
     *
     * Calcula policy (generated): Claim rectangles of its own HTML frame for clicks; clicks outside them still reach the grid, and Design Mode suspends them all.
     * Reach: broker `render.setHitRegions`, restricted tier, class mutate, requires the `ui.htmlInput` capability.
     */
