...
describe("partitionByFacet", () => {
  it("splits rows into one panel per distinct field value (first-seen order)", async () => {
    const facets = await partitionByFacet(GRID, ROWS, COLS, true, "columns", "Region", spec(), noLookups());
    expect(facets).toBeDefined();
    expect(facets!.map((f) => f.value)).toEqual(["North", "South"]);
  });

  it("builds each panel from only its matching rows", async () => {
    const facets = (await partitionByFacet(GRID, ROWS, COLS, true, "columns", "Region", spec(), noLookups()))!;
  ...
  it("runs transforms per panel (Vega-Lite facet semantics)", async () => {
    const transform: TransformSpec[] = [{ type: "filter", field: "Sales", predicate: "> 8" }];
    const facets = (await partitionByFacet(GRID, ROWS, COLS, true, "columns", "Region", spec({ transform }), noLookups()))!;
  ...
  it("panels do not nest (no recursive facets)", async () => {
    const facets = (await partitionByFacet(GRID, ROWS, COLS, true, "columns", "Region", spec(), noLookups()))!;
    for (const f of facets) expect(f.data.facets).toBeUndefined();
  });

  it("labels a blank facet value", async () => {
    const grid = [["Region", "Month", "Sales"], ["", "Jan", "10"], ["", "Feb", "20"]];
    const facets = (await partitionByFacet(grid, 3, COLS, true, "columns", "Region", spec(), noLookups()))!;
    expect(facets).toHaveLength(1);
    expect(facets[0].value).toBe("(blank)");
  });

  it("caps panels at MAX_FACETS", async () => {
    const grid: string[][] = [["Region", "Month", "Sales"]];
    for (let i = 0; i < MAX_FACETS + 10; i++) grid.push([`R${i}`, "M", "1"]);
    const facets = (await partitionByFacet(grid, grid.length, COLS, true, "columns", "Region", spec(), noLookups()))!;
    expect(facets).toHaveLength(MAX_FACETS);
  });

  it("returns undefined for an unknown field (→ caller renders single chart)", async () => {
    expect(await partitionByFacet(GRID, ROWS, COLS, true, "columns", "Nope", spec(), noLookups())).toBeUndefined();
  });

  it("returns undefined without a header row", async () => {
    expect(await partitionByFacet(GRID, ROWS, COLS, false, "columns", "Region", spec(), noLookups())).toBeUndefined();
  });

  it("returns undefined for rows orientation (v1: columns only)", async () => {
    expect(await partitionByFacet(GRID, ROWS, COLS, true, "rows", "Region", spec(), noLookups())).toBeUndefined();
  });

  it("does not apply index-based chart filters per panel", async () => {
    ...
    const facets = (await partitionByFacet(
  ...
  it("matches the field by trimmed header", async () => {
    const grid = [[" Region ", "Month", "Sales"], ["North", "Jan", "10"]];
    const facets = await partitionByFacet(grid, 2, COLS, true, "columns", "Region", spec(), noLookups());
    expect(facets).toBeDefined();
