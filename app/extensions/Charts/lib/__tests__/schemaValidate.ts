...
// PURPOSE: A small JSON-Schema (draft-07 subset) checker for tests — used by
//          the schema drift guard and the example-corpus validity test.
// CONTEXT: Not a *.test.ts file, so vitest does not run it as a suite. It only
//          implements the keywords chartSpecSchema actually uses: $ref, type,
//          const, enum, properties + required + additionalProperties:false,
//          items, oneOf/anyOf, allOf, and if/then/else. The goal is to catch
//          the two drift classes that produce false editor errors — unknown
//          properties and missing required fields — plus per-mark narrowing.
