...
describe("custom transform dispatch (registry)", () => {
  it("runs a registered custom transform and flows its output on", async () => {
    ...
    const out = await applyTransforms(baseData(), [custom("t:double")]);
    expect(out.series[0].values).toEqual([2, 4]);
  });

  it("warns (and leaves data unchanged) for an unregistered transform type", async () => {
    const diags: TransformDiagnostic[] = [];
    const out = await applyTransforms(baseData(), [custom("t:unregistered")], diags);
  ...
  it("degrades to a diagnostic + input data when a custom transform throws", async () => {
    registerChartTransform("t:throws", { apply: () => { throw new Error("boom"); } });
    const diags: TransformDiagnostic[] = [];
    const out = await applyTransforms(baseData(), [custom("t:throws")], diags);
    expect(out.series[0].values).toEqual([1, 2]); // unchanged
    expect(diags.some((d) => d.severity === "error" && d.message.includes("boom"))).toBe(true);
  });

  it("degrades when a custom transform returns a non-ParsedChartData value", async () => {
    registerChartTransform("t:bad", { apply: () => ({ nope: true }) as unknown as ParsedChartData });
    const diags: TransformDiagnostic[] = [];
    const out = await applyTransforms(baseData(), [custom("t:bad")], diags);
    expect(out.series[0].values).toEqual([1, 2]); // unchanged
    expect(diags.some((d) => d.severity === "error" && d.message.includes("invalid chart data"))).toBe(true);
  });

  it("rejects a series array whose elements lack values[] (would crash a downstream step)", async () => {
    ...
    const diags: TransformDiagnostic[] = [];
    const out = await applyTransforms(baseData(), [custom("t:bad"), { type: "sort", field: "S", order: "asc" }], diags);
    expect(out.series[0].values).toEqual([1, 2]); // unchanged; pipeline did not crash
    expect(diags.some((d) => d.severity === "error" && d.message.includes("invalid chart data"))).toBe(true);
  });

  it("passes resolved params through to the custom transform context", async () => {
    let seen: unknown;
    registerChartTransform("t:params", {
      apply: (data, _spec, ctx) => { seen = ctx.params?.get("Threshold"); return data as ParsedChartData; },
    });
    await applyTransforms(baseData(), [custom("t:params")], undefined, undefined, undefined, new Map([["Threshold", 42]]));
    expect(seen).toBe(42);
  });

  it("a custom type does NOT short-circuit built-in transforms in the same pipeline", async () => {
    registerChartTransform("t:double", {
      apply: (data) => {
        const d = data as ParsedChartData;
        return { ...d, series: d.series.map((s) => ({ ...s, values: s.values.map((v) => v * 2) })) };
    ...
    const out = await applyTransforms(baseData(), [
