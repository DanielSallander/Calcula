...
export const GLOBAL_INPUT_LISTENERS: readonly GlobalInputListener[] = [
  ...
  { file: "extensions/Charts/index.ts", event: "contextmenu", verdict: "right-press-exempt",
    note: "Opens the chart's own menu. `isPointerClaimed` deliberately never claims SECONDARY_MOUSE_BUTTON so an object's context menu still opens; guarding this with the CLAIM would be the trap the exemption exists to prevent. Two guards that are not the claim were missing and are now there: a right-click whose target is an INPUT/TEXTAREA/SELECT/contentEditable is that field's (the overlay text editor mounts a <textarea> over a chart title, and its paste menu was being replaced by the chart's), and a right-click outside the grid container's own box is nobody's chart — the chart REGION is not clipped to the visible canvas, so a chart wider than a task-pane-narrowed grid matched clicks inside the pane." },
  { file: "extensions/Charts/index.ts", event: "keydown", verdict: "claim-guarded",
    note: "THREE handlers, one row (file + event). (1) Delete/Backspace removes the selected chart, or just its selected furniture. (2) plain Left/Right steps the insight overlay's points of interest. (3) the arrows walk the chart's element groups and members, and Escape steps one level up. Each door asks `isKeyClaimed` here AND `chartOwnsKeystroke` (Charts/handlers/selectionHandler.ts), which asks the claim again plus `isGridFocused` — a claim is a grid-overlay concept, so a focused <button> in the Format pane or on the ribbon carries none, and with only the claim and a tag list Delete from inside the chart's own pane DESTROYED THE CHART. Arrow precedence between (2) and (3) is one pure rule, `arrowsBelongToOverlayStep`, that both read. Ctrl+1 is NOT here: it goes through the keybinding registry with a `when` predicate, because that listener pre-empts this one." },
  ...
  { file: "extensions/WatchWindow/components/WatchWindowDialog.tsx", event: "keydown", verdict: "session-scoped",
    note: "Escape / arrow keys for a popup or dialog this surface itself opened; it acts only on its own surface." },
  { file: "src/api/dialogLayout.tsx", event: "mousemove", verdict: "session-scoped",
    note: "Pane-divider drag: registered only while a drag this dialog's own splitter began is live, so the press that started it was already filtered." },
  { file: "src/api/dialogLayout.tsx", event: "mouseup", verdict: "session-scoped",
    note: "Pane-divider drag: registered only while a drag this dialog's own splitter began is live; it ends the drag and nothing else." },
  ...
  { file: "src/core/lib/dialogs.ts", event: "keydown", verdict: "session-scoped",
    note: "Escape / arrow keys for a popup or dialog this surface itself opened; it acts only on its own surface." },
  { file: "src/core/lib/overlayTextEditor.ts", event: "mousedown", verdict: "session-scoped",
    note: "Click-outside commit for an in-place editor this seam itself opened: registered for the life of ONE session, ends only that session, and stands down while the session is expecting a formula reference, because that press is a PICK. It is on WINDOW and in CAPTURE because a press on the grid is preventDefault()ed, so the editor never loses focus and no blur ever arrives — without it, an editor whose Enter types a newline has no commit gesture at all." },
