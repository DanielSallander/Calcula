...
// CONTEXT: Consents persist inside the workbook (.cala virtual filesystem) so the
//          user is not re-prompted on every open. Consent is keyed per package AND
//          per script source hash: if an upstream refresh changes a script's
//          source, the package re-prompts — silent code swaps must never inherit
//          consent. Promoted from the ScriptableObjects extension to @api so EVERY
//          distributed-code surface (object scripts via ScriptableObjects, the
//          sandboxed chart-transform/chart-mark libraries via Charts) reuses ONE
//          consent store rather than each inventing a parallel one.

import { readVirtualFile, createVirtualFile } from "./backend";
...
// SERIALIZE every recordConsent: it is a read-modify-write of one shared file
// (.calcula/script-consent.json) and there are now TWO independent writers — the
// ScriptableObjects object-script gate AND the Charts chart-library gate. Without
// serialization, two concurrent grants both read the same baseline and the second
// write clobbers the first's just-added record (lost update). Chaining on one
// module-level promise makes each grant's loadConsents() observe the prior write.
 ...
 * durable once the workbook is saved. Serialized against all other recordConsent
 * calls so two concurrent grants (object scripts + a chart library) can't drop
 * each other's record.
 */
