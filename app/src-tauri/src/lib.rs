...
use engine::{
    ...
    CellError, CellStyle, CellValue, EvalResult, Evaluator, Grid, NumberFormat,
...
// ============================================================================
//
// The ladder itself now lives in `core/engine/src/typed_entry.rs`. It moved so
// that the offline formula grader — which lives under `core/` and cannot depend
// on this crate — seeds a fixture through the SAME code the product uses for a
// typed cell, rather than through a second implementation that would drift.
//
// Re-exported under the names it always had, so every call site here is
// unchanged.
pub use engine::typed_entry::{
    ...
    parse_date_time_input, parse_number,
};
