 ...
 */
/**
 * Which schema to measure against.
 *
 * `sales_star` is the original, and EVERY recorded design-query number is about
 * it. `renamed_star` is the same star with the same rows and the same 40
 * questions under a different vocabulary, generated by
 * `tests/fixtures/model/gen-renamed-star.mjs`. The gap between the two is the
 * thing worth knowing: it says whether a score is about the DSL or about having
 * seen these particular column names.
 */
const fixture = String(arg("fixture", "sales_star"));
...
const bundle = JSON.parse(
  readFileSync(path.join(repo, "tests/fixtures/model", `${fixture}.json`), "utf8"),
);
const strategyDoc = JSON.parse(
  readFileSync(path.join(repo, "tests/fixtures/model", `${fixture}_strategy.json`), "utf8"),
);
const biModel = { ...modelInfoFromFixture(bundle), strategy: strategySummaryFromFixture(strategyDoc, bundle) };

// The corpus travels WITH the fixture: `renamed_star` has its own 40 tasks in
// its own vocabulary, and running the original intents against it would ask for
// columns that no longer exist.
const corpusFile =
  fixture === "sales_star" ? "design-queries.json" : `design-queries-${fixture.replace(/_star$/, "")}.json`;
const corpus = JSON.parse(readFileSync(path.join(here, corpusFile), "utf8"));
