...
- **Time intelligence**: YTD/QTD/MTD running totals, PRIORYEAR/SAMEPERIODLASTYEAR/PRIORPERIOD/PARALLELPERIOD shifts, and DATESINPERIOD trailing windows against a marked date table, evaluated either from the group-by axis or purely from the filter context (no date column on the axis — period shifts there work at year, quarter, and month granularity), failing closed rather than returning a silently-wrong value when the window cannot be computed
...
Beyond that baseline, the engine adds the host-facing capabilities catalogued in [host-integration-changelog.md](host-integration-changelog.md): presentation metadata; pivot-shaped results (`order_by`/`limit`/sort-by-column and `ROLLUP` totals with a `__grouping_id` indicator); multi-select (`in_filters`) and cross-column `OR` (`or_filters`) slicers; `&self` concurrent queries with cooperative cancellation; **time intelligence** (axis and filter-context modes); **row-level security** (single role or multi-role union); **drillthrough / detail rows** with dimension-attribute output; **incremental refresh** (user-defined volatility filter) with structured `RefreshReport`; **calculation groups**; and **scripting** (native UDF registry + sandboxed Rhai script functions). The shared model file is now a hardened trust boundary with `MODEL_FORMAT_VERSION` evolution (currently `8`). Across these waves the overriding rule is **never return a silently-wrong number — fail closed instead**.
