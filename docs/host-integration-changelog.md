...
> **Known limitations (deferred):** in **axis mode**, an outer `KEEP` around a single window measure now applies (a non-date filter restricts the running total); a `KEEP` around a *compound* one fails closed. Fiscal calendars (non-Gregorian date tables) fail closed on the filter-context path. **`TotalsMode::Rollup` and ragged hierarchies compose with the full filter-context family** — `YTD`/`QTD`/`MTD`, `DATESINPERIOD`, `CLOSING`/`OPENINGBALANCE`, `PRIORYEAR`/`PRIORPERIOD`/`PARALLELPERIOD`, and compound YoY (see the [Rollup result contract](#totalsmoderollup-result-contract)). Axis-mode windows/period-shifts (a date column on the group-by axis) and value-based (gap-tolerant) period shifts remain deferred (they error rather than mislead).
