...
| **Right-click does not move the chart's selection, so a menu item acts on whatever the last LEFT click selected.** FILED 2026-09-21. `ChartContextMenuContribution` carries only a `chartId` — `visible?(chartId)` and `onSelect(chartId)` — so a contribution cannot know which bar was right-clicked. Right-clicking bar B while bar A is selected therefore acts on A, which is the owner's finding in its general form for anyone whose model is "right-click the thing I want". The cheap shape is NOT to widen the contribution contract (that touches all six existing contributions and changes the menu's subject for every one of them): `getHoverState()` already carries the datum under the cursor and the right-click handler already reads it for the axis menu, so Charts can record "the datum this menu was opened on" and expose it through the `ChartDataProvider` seam it already owns, beside `getSelectedChartId()`. The item then reads `the right-clicked datum ?? the selected datum`. | `app/src/api/chartContextMenu.ts`, `app/src/api/chartData.ts` (`ChartDataProvider`), `app/extensions/Charts/index.ts` (`handleContextMenu`, `getHoverState`) |
