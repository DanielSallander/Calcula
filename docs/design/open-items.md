...
| **Aborting a pending JOB corrupts the QuickJS runtime, and the notebook session is reused afterwards.** Measured 2026-08-20: a cell whose continuation spins (`(async () => { await null; for (;;) {} })()`) is cut by the deadline interrupt DURING job execution, and QuickJS is then left holding a bad refcount — dropping that runtime trips its own `p->ref_count > 0` assertion and takes the process down with `STATUS_STACK_BUFFER_OVERRUN`, after the test harness has already printed a green result. Reproduced in-suite; the regression test for the timeout drain has to `std::mem::forget` its session to keep the two hazards apart. **This became REACHABLE only once jobs began running at all**: before `drain_jobs`, a queued continuation never executed, so it could never be interrupted — the fix traded a silent no-op for a loud crash in the runaway case, which is the better trade but not a free one. The real repair is host-side and small: `notebook_executor.rs` reuses the same `NotebookSession` after a failed cell (`let s = session.as_ref().expect("session just ensured")`), so a timed-out cell should DISCARD the session and build a fresh one — which also makes any stranded job moot. | `core/script-engine/src/runtime.rs` (`drain_jobs`), `core/script-engine/src/notebook.rs` (`a_timed_out_cell_leaves_no_job_behind_for_the_next_cell`), `app/src-tauri/src/scripting/notebook_executor.rs:303-305` |
| **An async handler in the EXTENSION worker realm fails completely silently.** `dispatchAppEvent` does `void fn(payload)`, so an `async` handler's throw becomes a rejected promise and the surrounding `catch` is dead code for every async handler — and every privileged call in that realm goes through `brokerCall`, which returns a Promise, so any handler that does real work IS async. Nothing observes the rejection: `extensionBootstrap.ts` registers no `unhandledrejection`, and the frontend's only such listener (`worker/bootstrap.ts:489`) belongs to the OBJECT-SCRIPT realm and early-returns unless a debug session is live. A sync throw yields `console.error("[ext:…] uncaught:")`; an async one yields nothing anywhere. **The object-script twin already has the fix** — `worker/contextShims.ts:683` collects thenables and reports rejections like synchronous throws — and its own doc comment names this exact hazard; the extension twin never received it. The sibling method in the SAME file (`invokeHandler`, line 733) does `await fn(...)` correctly. | `scriptHost/worker/extensionWorkerContext.ts:748`, cf. `worker/contextShims.ts:683-718` |
| **A failed extension menu-item click is indistinguishable from a successful no-op.** `void invokeWorkerHandler(...)` discards a promise that rejects on three real failures — the handler threw (`:506`), the 5s invoke timeout (`:1258`), and unmount mid-flight (`:422`). The `action` handed to `registerMenuItem` is a sync arrow returning void, so the menu dispatcher cannot observe anything either, and with no window-level `unhandledrejection` listener the rejection is swallowed whole. The user clicks an add-in's menu item, it fails, and the UI shows nothing: no toast, no console line. MenuBar's `commandId` path DOES catch; only the `action` path does not. Same shape one line up at `void CommandRegistry.execute(...)`. | `scriptHost/extensionWorkerHost.ts:817` |
