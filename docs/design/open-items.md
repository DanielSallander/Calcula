...
| **The in-app AI chat cannot hand the user a script to review — only an external MCP client can.** `draft_object_script` / `list_script_drafts` / `get_script_draft` are registered on the MCP server (`server.rs:1069,1098,1110`), which exposes **37 tools**. The in-app chat declares **21** (`ChatView.tsx:20`), its dispatcher has **exactly 21 matching arms** (`ai_chat.rs:230-345`), and none of the three is among them. So the chat's only route to a script is `run_script`, which **executes immediately** — the inverse of the review-then-mount invariant `drafts.rs` exists to hold, and the reason that module deliberately keeps drafts out of `AppState.object_scripts` (where `loadAndMountScripts` would run them on the next open). Three match arms plus three tool declarations. **This is not local-model work**: it is the correct behaviour with Claude today and should not wait on anything. Counts were extracted by parsing, not `grep -c` — an earlier pass reported 152 ops / 29 capability ids from line-counting and both were wrong. Design: `docs/design/local-model-script-authoring.md` §3b, M1. | `ai_chat.rs:230-345`, `ChatView.tsx:20`, `mcp/server.rs:1069,1098,1110`, `mcp/drafts.rs:238` |
| **The in-app AI chat is hard-locked to one vendor, and offers no model picker at all.** Four shallow pins: the endpoint is a const (`ai_chat.rs:33`); the credential target is the single fixed string `Calcula:aikey\|anthropic`, so only ONE vendor's key can be stored at a time (`:30`); auth is Anthropic's `x-api-key` + `anthropic-version` rather than `Authorization: Bearer` (`:174-179`); and `thinking` is gated on `claude-opus-4` (`:153`). The **structural** pin is the fifth: `ChatView` builds the Anthropic wire format itself — `input_schema` tools, `stop_reason === "tool_use"`, `tool_use_id` result blocks (`ChatView.tsx:20,326-357`) — putting one vendor's JSON schema inside an extension, against the Facade Rule. `ai_chat.rs`'s own header states this as deliberate ("the frontend speaks the Anthropic wire format, so the Tauri layer stays thin"), so it is a decision to revisit, not an oversight to patch. **And the model is not selectable even within Anthropic:** `ai_chat_complete` accepts a `model` parameter, `ChatView` never passes it (`:327-331`), so every request is `DEFAULT_MODEL = "claude-opus-4-8"` (`:32`), and the setup UI hardcodes "Connect Claude" with an `sk-ant-…` placeholder (`:369,374`). Owner intent (2026-08-19) is VS-Code-Copilot-style free choice of **any** model — local or cloud, any vendor. One `openai_compat` provider covers every local runtime *and* most cloud vendors; Anthropic keeps a native impl. Design: `docs/design/local-model-script-authoring.md` §3c, §7, M3. | `ai_chat.rs:30,32,33,153,174-179`, `ChatView.tsx:20,261,327-331,369-374` |
