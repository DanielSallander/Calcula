...
| item | verified at |
|---|---|
| **The editor LIVE PREVIEW of a form now exists; what it cannot yet show is a BOUND value from a range or a control.** `lib/formPreviewBridge.ts` runs the draft through the Worker-realm preview, takes the layout the harness captured from `form.define`, reads the bound cells back out of the SAME snapshot the run used, and opens the renderer in preview mode (`preview: true`), where Submit lists what would be written instead of writing it. Two seeding gaps remain by construction: a `{ range }` choice list and a `{ control }` binding are resolved by the HOST at show time, and the preview has no host — those widgets paint unbound with a reason. Closing them means teaching the preview backend to serve the range read against the snapshot. | `extensions/ScriptableObjects/lib/formPreviewBridge.ts`, `scriptPreview/backend.ts:766-772`, `scriptPreview/report.ts:28-41`, `ScriptFormDialog.tsx` (preview branch) |
| **The form journeys are WRITTEN but have never been RUN.** `app/e2e/journeys/script-form.spec.ts` and `script-form-distributed.spec.ts` exist and pass `check-types` and eslint, and `script-preview.spec.ts` has a form case — but no live run has happened in this session, so every selector and every backend assumption in them is unproven. The authoring agent listed what it could not settle from source: whether `calp_publish` succeeds for a workbook with no BI model, the sv-SE rendering of the `currency_usd` preset, and whether a `<select>` whose controlled value matches no `<option>` reports `""`. Run them before treating any of it as cover. | `app/e2e/journeys/script-form.spec.ts`, `app/e2e/journeys/script-form-distributed.spec.ts`, `app/e2e/journeys/script-preview.spec.ts` |
...
| **Package inspector shows a form as emitted JavaScript, not as a rendered layout.** `ScriptsSection` renders every distributed script's full `source` in a `SourceBlock`; for a form that is a nested object literal in a `form.define` call, which is faithful (ONE ARTIFACT) but is not what a reviewer deciding whether to trust a form wants to see. A "preview layout" action in the inspector — the same `ScriptFormDialog` in preview mode fed by the layout the harness can capture from the package's source — is the follow-up. It must run the source through the preview harness (a strict snapshot, empty ceiling), never mount it. | `extensions/Distribution/components/inspector/ScriptsSection.tsx:96-107,173`, `scriptPreview/index.ts:241-272` |
| **M2 — modeless floating window and task pane needs a NEW capability id.** `ui.dialog`'s four user-facing sentences promise "a dialog you must answer or close before continuing"; a modeless surface makes that false, so it is `ui.pane` (or similar) with the full lockstep: `ALL_CAPABILITY_IDS`, `CAP_DESCRIPTION`, both phrase tables, the audit classification map, the `scriptSurfaces.ts` rows, and Rust `KNOWN_CAPABILITY_IDS` (currently `[&str; 16]`). Three prerequisites the code shows: sessions must become script-visible instances (the shim already keys waiters by `showId`); there is NO event backpressure — `EVENT_QUEUE_HIGH_WATER` is declared and read nowhere in `app/src`; and no panel host exists — `ExtensionPanelHost` from the add-in design was never built. | `capabilityIds.ts:216`, `capabilities.ts:411`, `SubscribeDialog.tsx:63`, `inspector/ScriptsSection.tsx:46`, `broker.ts:295`, `core/persistence/src/lib.rs:1705`, `contextShims.ts:782-816`, `protocol.ts:426`, `docs/design/third-party-addin-authoring.md:361` |
...
| **The bound-cell READS a form performs happen before the show guards, so a refused show still read the sheet.** `resolveFormBindings` runs in the `form.show` executeImpl case BEFORE `showScriptForm` applies the mute, the 20-per-minute show bucket and the app-wide modal slot — so a muted script calling `show()` in a loop still performs (and audits) a read per bound cell for a dialog that never opens. Nothing UNAUTHORISED happens: every read is a broker call under the script's own handle with the tier clamp and the audit row it would have anyway. It is wasted IPC and audit noise. The fix is to let the registry decide first — a non-consuming pre-check, or a `resolve` thunk showScriptForm awaits after its guards pass. | `host.ts` (case "form.show", case "cap.formsShow"), `scriptForms.ts` (showScriptForm guards) |
| **Doc drift: `CLAUDE.md` says the `.cala` format version is 7; the code is at 8.** The "`.cala` Format Versioning" paragraph reads "currently at **7**" and lists the per-feature minimums up to `SPILL_EXTENT_MIN_FORMAT_VERSION` (7), while `CALA_MAX_SUPPORTED_FORMAT_VERSION` is `8`. Not touched by the forms work (a form adds no version); the sentence and the reason for version 8 need to be written by whoever bumped it. Both figures were read on 2026-09-03. | `CLAUDE.md:223`, `core/calcula-format/src/manifest.rs:57` |
