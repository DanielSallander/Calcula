...
| ~~**The two `evaluate-formula` goldens assert residue, not their feature.**~~ **CLOSED 2026-08-17.** Both are now region captures of the cells the spec itself writes (`AI1:AI3` and `AI9`) instead of whole-grid shots in which 26 of 27 framed columns belonged to other specs — the visible content and even the percent format on AI2 were `edge-cases.spec.ts`'s residue. `takeGridRegionScreenshot` frames the range and parks the selection away from it, so the `navigateTo` that was part of the problem is gone. **ACTION REQUIRED — 2 tests are RED until the baselines are re-recorded, and it could not be done on 2026-08-17.** The capture-environment guard refused: this session's display reported `devicePixelRatio` 1 while every committed golden was recorded at **2** (a 200% display). Recording there would have silently invalidated the whole 72-golden corpus, so the guard is right to refuse. On a 200% display this is one command:

```
cd app && E2E_MANUAL=1 npx playwright test --project=functional \n  e2e/tests/evaluate-formula.spec.ts --update-snapshots
```

To defer instead, restore the two deleted PNGs and revert the two capture calls to `takeGridScreenshot`:

```
git checkout -- app/e2e/tests/__screenshots__/evaluate-formula.spec.ts/
```

Do NOT pass `--reporter=...` on that run: it replaces the reporter list and the collection guard fails the run by design. `softly()` does not suppress a MISSING snapshot, which is deliberate — it forces the re-record to be a decision rather than a silent pass. | `e2e/tests/evaluate-formula.spec.ts` |
