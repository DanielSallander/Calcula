...
reproduction live in `tests/regression/bug-ledger.json` (**125 entries, 123 fixed, 2 open** as of
2026-09-21 — recounted from the file with node, not carried forward; it moved three times in two
days once already, and the figure this sentence carried on 2026-09-13 said 106/104/2, which was 7
entries and 2 open bugs behind; the 2026-09-13 recount of 113/111/2 has since been overtaken by
BUG-0114 through BUG-0125, with no gaps; the last three are the chart-interaction waves —
BUG-0123 the `axis.reverse` no-op, BUG-0124 and BUG-0125 the two Delete-key data-loss defects in
`docs/design/chart-interaction.md` §6.2). **The two open are unchanged**, and none of the twelve
entries added since is open: **BUG-0098** (the unreproduced backend wedge in §2.5, kept open
...
| ~~**`query_auto_refresh` / `query_auto_tier` / `query_explained` apply no object-level security, and the first two silently ignore `rank_by` / `top_n` / `measure_filters`.**~~ **CLOSED 2026-09-18 — one aggregate query path** (`model-engine-lib` changelog entry "One aggregate query path"). `query_auto_refresh` is now literally the pre-checks + `refresh_stale` + `query_with_cancellation`; `query_auto_tier` is the pre-checks + tiering + TTL refresh + `query_with_cancellation`; `query_explained` runs OLS, the post-aggregation peel and the multi-role union ahead of the explained plan (and reports a peeled step as a `PostAggregation` node). So OLS, `rank_by` / `top_n` / `measure_filters`, the multi-role union, GVAR/ISFILTERED resolution and context-column pushdown apply identically on every aggregate entry point; `reject_multi_role` guards only drillthrough. The design critique found two more holes closed in the same change: the OLS gate ignored **scoped filters** (the shape every host filter arrives in — a caller could bisect a denied column through `WHERE Employees[Salary] > x`, on `query` too; BUG-0121), and `query_auto_tier` ran a multi-role request with EMPTY role filters (no host caller). Host side: the filter pane picks the first placeholder measure the role permits under one lock (`distinct_values_request` + new `Engine::check_object_level_security`), `friendly_bi_query_error`'s dead RLS branches fixed, CUBE logs the refusal, Model Editor "include plan" returns the same rows as without. FILED 2026-09-17 while fixing the derived-measure home-table refusal. The adversarial review of the change (4 lenses, 2 refuters per finding) confirmed one more host defect and it was fixed here: `bi_refresh_connection` rewrote one grid block per active query and recalculated dependents only after the loop, so a refusal on the second query left the first block rewritten with its formulas holding the previous refresh's numbers — the `sort_range`/`clear_range` class. It now pre-checks every active query's objects in one lock before touching the grid, and carries a later failure past PHASE B instead of returning through it. **A follow-up round (2026-09-18) then swept the whole BI error surface for silent failures and closed the worst of it** — see the row below for what it found and what is still open. From THIS change specifically, one residual remains: the filter pane's placeholder measure can change which fact table a slicer's member list joins through when a role denies the first measure (declared order is kept deliberately, and the doc says so); and an engine that has auto-tiered tables now plans them local on the plain `query` path too (freshness only; Calcula never tiers). Small, same area, from the adversarial review of that fix (all pre-existing, none introduced by it): a CONSTANT measure (`BLANK()`, `42`) is now refused by the planner by name — homing a constant that stands BESIDE a real measure on that measure's table is a follow-up, and a dimension-only BI pivot picks `meta.measures.first()` as its synthetic measure (`pivot/commands.rs` ~6258), so a placeholder constant as the FIRST model measure blanks such a pivot silently; `resolve_measure_home_tables` is fill-only, so a dependent keeps its cached home table if its base measure is later moved to another table by a Model Editor edit of an already-resolved model (the ordinary path re-resolves from the unresolved `base_model` clone, so it is an edge); the host's field-list OLS filter tests only the first-wins home table (`pivot/commands.rs` ~5662) while the engine's query gate walks the whole closure; `insights/strategy/facts.rs` treats an empty `fact_table` (a constant measure) as a real table with no relationships, which yields a note with an empty table name; and a pivot's persisted `MeasureFieldInfo.table` is extracted only at creation, so pivots saved before the fix keep `""` for derived measures until re-created (no backward compatibility owed). | `model-engine-lib/crates/engine/src/lib.rs` (`query_auto_refresh`, `query_auto_tier`, `enforce_object_level_security`), `crates/engine-query/src/planner/pushdown/mod.rs`, `app/src-tauri/src/pivot/commands.rs`, `app/src-tauri/src/insights/strategy/facts.rs` |
| **The insight overlay took the click before the chart's own selection ladder saw it, so a ringed bar could never be selected and a comment landed on a bar the reader had left.** FOUND LIVE 2026-09-21 by the owner, two reports with one cause. **CLOSED.** The overlay's click branch selected a cue and RETURNED (so `advanceSelection` never ran for a ringed bar — chart/series/dataPoint was unreachable on exactly the bars the lens points at), and it assigned only when a cue was HIT (so a click on an un-ringed bar left the previous ring selected, and "Add comment on this point…" wrote onto it — an undoable document mutation, the §5g defect in a second guise). The branch no longer returns and the assignment is unconditional: a click that lands on no cue CLEARS the selection, the way a click on the plot background drops the ladder to chart level. `deselectChart` clears it too, for the chart being left only. **The owner's own suggestion — hit-test the cue's OUTLINE and leave the bar alone — was priced by a ten-agent review and refused on the repo's own constants:** `CUE_RING_PAD` 4 needs 8px of clearance, `createBandScale`'s 0.2 padding gives `bandwidth * 0.2` between bars (11.1px at 6 categories, 5.6px at 12, 2.8px at 24, 1.7px at 40), so above ~9 categories the ring's side segments are painted ON TOP of the neighbours and the target does not exist — and whether it exists depends on category count, grouping and stacking, which the reader cannot see. It also leaves a comment welded to a cue, so it answers half the report. **One regression came with the fix and was closed in the same change:** dataPoint level fills every OTHER element `rgba(255,255,255,0.55)`, and the overlay paints AFTER the highlight, so selecting one marked bar left the other marked bars pale under their own rings. The wash is skipped while the chart carries visible cues. The three painters moved to `selectionHighlight.ts` to get a unit tier (they had none); threading the flag inverted the PIE painter's branches so every slice was outlined when dimming was off, caught by the new test before anything ran. **The proof that was missing:** both defects shipped green because the journey selected cues programmatically and its only chart mouse events were right-clicks; it now drives real LEFT clicks located from the renderer's own hit geometry. Five sabotages. **Live run owed** — it refused at the capture-environment guard, `devicePixelRatio` 2 against the corpus's 1 (the 200% laptop panel, not the 100% external display). | `app/extensions/Charts/index.ts` (the mouseup cue branch), `app/extensions/Charts/handlers/selectionHandler.ts` (`deselectChart`), `app/extensions/Charts/rendering/selectionHighlight.ts` (new), `app/extensions/Charts/rendering/__tests__/selectionHighlight.test.ts`, `app/extensions/Charts/handlers/__tests__/selectionHandler.test.ts`, `app/e2e/journeys/insight-overlays.spec.ts` |
| **A chart comment is still welded to a cue, so a bar with no point of interest on it cannot take one.** FILED 2026-09-21, the half of the owner's report that the ladder change does not close. `ChartCueComment` carries `cueId` and takes its anchor from `datumOfCue`, so "Add comment on this point…" is offered only while a ring is selected. The recommended shape, from the same review: the comment anchors to a DATUM, with `cueId`/`factId` made NULLABLE rather than removed — every record already on disk then still validates, including the `anchor: null` tray records that a non-null anchor would silently drop on load. That risk is real, not theoretical: `publish()` writes the re-anchored list into `commentsByChart` and `persistPayload()` serialises it wholesale, so re-anchored state is STAGED for disk and the next write of anything in the blob flushes it — including `saveOverlayStyle`, which shares it. Re-anchoring would resolve by (series, categoryLabel), keeping a comment on its month through a filter and putting it in the tray when the month leaves the chart; on the document-mutating keep path an ambiguous label must REFUSE rather than take the first match. Two decisions ride on it: (a) a comment on a plain bar cannot follow a fact, so the "was Aug" badge goes away and D-IO-10 (marked unconfirmed in its own doc) is reversed — RECOMMENDED, because the current rule relocates the reader's words onto data they never looked at; (b) once a comment is about a BAR rather than a point of INTEREST, its home in the Insights lens store is justified only by history, and Charts already persists annotations in the spec. Also filed alongside: `cueAtDatum` resolves by first-match-in-array-order and the ladder change now runs it on every datum click, so it wants a DECLARED precedence (datum, span, series) in `@api`; "Keep this mark in the chart" is offered for cue kinds `keepCue` then refuses with a toast; and a `band` cue can be selected but has no selected appearance at all (`paintOne`'s band branch never reads `extra`). | `app/src/api/chartCues.ts` (`ChartCueComment`), `app/extensions/Insights/lib/overlayComments.ts`, `app/extensions/Insights/lib/overlay.ts` (`persistPayload`), `app/extensions/Insights/lib/overlayMenu.ts`, `app/extensions/Charts/rendering/cuePainter.ts` (`cueAtDatum`, `paintOne`) |
| ~~**Right-click does not move the chart's selection, so a menu item acts on whatever the last LEFT click selected.**~~ **CLOSED 2026-09-21 — and the cheap shape proposed here was only half of it.** The row's own fix (record the right-clicked datum beside the seam) is built: `ChartRightClickTarget` + `setChartRightClickTarget` / `getChartRightClickTarget` (`app/src/api/chartData.ts:198-256`), written by the chart's `contextmenu` handler for EVERY right-click, `null` included (the early return for a right-click that hit no chart, and the axis-menu branch), so a menu opened over the GRID is never handed the chart the reader right-clicked before it; `getChartRightClickTarget(chartId)` refuses a target belonging to another chart (`chartData.ts:252-255`). But recording the subject alone would have left the SELECTION still pointing at bar A while the menu acted on bar B, which is two subjects for one gesture. **Excel moves the selection on a right-click, so that is what the chart's `contextmenu` handler now does** (`handleContextMenu`, `app/extensions/Charts/index.ts`), in three stated steps: resolve the element from the CURSOR by bounds (`findChartAtCanvasPos` then `hitTestGeometry` — never from `getHoverState`, which is rAF-throttled and tracks only data elements and axes, so the same pixel answered two different questions depending on how fast the mouse got there); MOVE the selection exactly as a left-click on that pixel would, `advanceSelection` included so a first right-click on a bar selects the SERIES and a second selects the bar (the block headed `1. MOVE THE SELECTION`); then RECORD the subject, after the selection has moved, so the menu's rung and the selection's rung are the same rung (the block headed `3. RECORD THE SUBJECT`). Three consequences worth keeping: `pointIndex` is taken from the LADDER, not from the hit (the ladder-derived `painterPoint`, not `hit.pointIndex`) — the hit names a point on every datum click, and an ABSENT `pointIndex` is Excel's `PointIndex = -1`, which is the one thing deciding "Format Data Series..." vs "Format Data Point..."; the recorded pair is translated to AUTHORING space with `toAuthoringIndices` (immediately before the `setChartRightClickTarget` call), because `dataPointOverrides` are keyed pre-filter while the hit test answers post-filter, and skipping it is wrong only on a FILTERED chart, which is exactly how it would have passed review; and the record is deliberately NOT cleared when the menu unmounts, because `onClose()` runs BEFORE a contribution's `onSelect` (`chartData.ts:193-197`). The axis keeps its own menu and CLEARS the record rather than leaving it aimed at a menu nobody is reading (the `xAxis`/`yAxis` branch, which calls `setChartRightClickTarget(null)` before `showOverlay`). `CHART_TARGET_ELEMENTS` (`chartData.ts:158`) is a deliberate second spelling of `CHART_ELEMENT_IDS`, because `@api` must never import from `app/extensions`; it is pinned by a drift guard in `ChartContextMenu.test.tsx` asserting set equality in BOTH directions plus a compile-time assignability check each way. Full write-up: `docs/design/chart-interaction.md` §6.1. | `app/src/api/chartData.ts:198-256`, `app/extensions/Charts/index.ts` (`handleContextMenu`), `docs/design/chart-interaction.md` §6.1 |
| **A chart cue had no identity of its own, so a comment, a kept mark and the selection all landed on the wrong bar.** FOUND LIVE 2026-09-18 by the owner, commenting on the 2023 bar and watching the comment appear on 2025. **CLOSED in the same round.** An `extremes` fact emits TWO cues — the highest bar and the lowest — and `factId` was the only identity a cue had, so clicking either bar stored the same value and `getSelectedChartCue` returned the FIRST match. Everything downstream inherited it: the comment anchored on the wrong cue, **"Keep this mark in the chart" persisted a marker onto the wrong bar** (an undoable document mutation, so the defect reached saved files), and `reanchorComments` — also fact-keyed — dragged a correct comment back there on the next data change. `cuesForChart` now stamps `cueId` from the fact's id and the cue's DRAFT ordinal (stable when an earlier draft fails validation, which a placed-counter would not be); `selectedFactId` became `selectedCueId`; `ChartCueComment` carries `cueId` and resolves by it; the painter's selection highlight, the hover compare and the comment painter's polarity lookup are all cue-keyed. `setSelectedChartCue` REFUSES a fact id rather than guessing, and `firstCueOfFact` is the one sanctioned place a fact stands in for a cue (the stepper, which steps by fact by design). A persisted comment with no `cueId` is dropped on load: it can never re-anchor, so it would sit in the tray forever looking like the data changed. Four sabotages, each redding only its own guard — including one aimed at a NO-OP first (the getter's strictness is unreachable behind the setter's) and re-aimed at the rule that is actually observable. | `app/src/api/chartCues.ts`, `app/src/api/insightCues.ts` (`cuesForChart`), `app/extensions/Insights/lib/overlayComments.ts` (`datumOfCue`), `app/extensions/Charts/rendering/cuePainter.ts`, `app/extensions/Charts/rendering/cueChrome.ts`, `app/extensions/Charts/index.ts` |
| **A pivot table's cells were analysed as a range of numbers, producing "Grand total constitutes 50% of all revenue".** FOUND LIVE 2026-09-18 by the owner. **CLOSED.** A grand total is a row in the same column as the rows it sums, so the contribution fact reads it as a peer; the same goes for subtotals and for a row-header column of member names read as a category. `insights_analyze_range` now refuses any rectangle TOUCHING a `"pivot"` protected region, checked twice — as asked, and again after `plan_region` has expanded a single cell, because a click beside a pivot expands into it (that second check is the one a pre-check alone misses, and it has its own test). The rectangles are snapshotted from `protected_regions` with no other lock held, the discipline that module's header already demands for the hidden-row sets. `"bi"` regions are deliberately NOT refused: a model refresh target is an ordinary rectangle of numbers. The guard is in Rust so it covers the pane, the palette, the grid menu, MCP and the sheet overlay's non-model pivot fallback at once, and the message names the route that does work (a pivot over a MODEL is still analysed from the model's own facts). The grid menu hides "Analyse this range…" over a pivot so the reader is not offered an action that can only be refused. Four sabotages. | `app/src-tauri/src/insights/commands.rs` (`PIVOT_RANGE_REFUSAL`, `refuse_if_in_a_pivot`), `app/src-tauri/src/pivot/operations.rs` (`pivot_rects_on_sheet`), `app/extensions/Insights/index.ts` (`selectionTouchesAPivot`) |
| **The Insights pane's Analyse answered about the last clicked CELL while a chart was selected.** FOUND LIVE 2026-09-18 by the owner. **CLOSED.** Selecting a chart is deliberate and it is the most recent act, so it names the subject: `analyzeCurrentTarget` prefers the selected chart over the grid selection, and the pane's target line says so BEFORE the press. The model switch still beats both — that is a position the reader SET, not an incidental selection. A chart id the data provider does not list falls back to the range rather than naming a chart whose numbers cannot be resolved. The palette command and the pane button share the one function, so they cannot come to disagree about what "analyse" means. | `app/extensions/Insights/lib/chartExplain.ts` (`analyzeCurrentTarget`, `selectedChartTarget`), `app/extensions/Insights/components/InsightsPane.tsx`, `app/extensions/Insights/index.ts` |
| **The chart quick-access strip was a hard-coded array, and its click envelope was clamped to the CHART's height.** FILED and **CLOSED** 2026-09-18, opening the strip so Insights could put "points of interest" and its snapshot there as the owner asked. The strip took a seam (`@api/chartQuickActions`): a contributor declares an id, an icon from a CLOSED set, a per-chart tooltip and optional visibility/pressed predicates; Charts decides size, position, hover, pressed look, hit area and the icon's pixels. No `draw(ctx)` callback is accepted — a contributor handed the grid canvas could paint anywhere on it. Predicates are asked at paint time (the toggle's state is per chart) and one that throws costs that button, never the strip. The latent bug found beside it: `isInQuickAccessArea` used the chart's own height, so on a chart shorter than the strip the lowest buttons drew, hovered nothing and let the click fall through to the grid — deselecting the chart they belonged to. Three buttons made that a ~90px corner case; a fourth and fifth make it ordinary. The envelope is now the taller of the chart and the strip, and only in the strip's COLUMN, because the cells under a chart belong to the grid. | `app/src/api/chartQuickActions.ts`, `app/extensions/Charts/rendering/quickAccessButtons.ts` (`quickAccessStripHeight`, `quickAccessButtonKey`), `app/extensions/Charts/rendering/chartRenderer.ts`, `app/extensions/Insights/lib/overlayMenu.ts` |
| **The default cue was a blue oval on a blue bar.** FOUND LIVE 2026-09-18 by the owner, who asked for red squares. **CLOSED as squares, in charcoal.** A bar is a rectangle, and an oval around one leaves four wedges of background inside the mark while its sides cut across the bar's; the `rect` case now traces a box (a line or scatter point stays round, because it is round). The RED was declined and the owner accepted the pushback: red is `bad` in this palette and the standing rule is that red is never inferred from a falling number — the default overlay is exactly the case where nothing declares a direction. The neutral colour moved from `#0e639c`, the default bar's own blue, to charcoal `#3c4043`. The KEPT-mark painter draws the same box, so "Keep this mark" does not change the picture the reader was looking at when they pressed it. `rect`/`closePath` were deliberately kept OUT of `CuePaintContext`: the surface the painter needs is the surface a test has to double. | `app/extensions/Charts/rendering/cuePainter.ts` (`pathRect`), `app/extensions/Charts/rendering/markerPainter.ts`, `app/src/api/insightStyle.ts` |
| **A member cue can lose the period it is about, and land on that member's GRAND TOTAL — silently.** FOUND LIVE 2026-09-18 by `app/e2e/journeys/insight-overlays-pivot.spec.ts` on the sales-star fixture, and **half-fixed in the same change.** A contribution/memberMove fact carries no period of its own, so the cue mapper anchors it to the measure's last period borrowed from another fact. That anchor came from the **change fact alone** (`pivotCues.ts`, the `lastLabelByMeasure` harvest). The engine CAPS a run at `MAX_FACTS = 12` and ranks by score, so once the derived-measure fix (BUG-0119/BUG-0120) made Margin/MarginPct/Cost/Quantity analysable, their contribution facts crowded `change:m/Revenue` (score ~0.626) out of the budget while `variance:m/Revenue` (~0.754) survived — every Revenue member fact then lost its period requirement, matched on ONE pair, and landed on that member's grand-total cell: a number that had not moved, marked as the movement it is not. It fails SILENTLY (the cue is placed, not dropped, so no `PivotCueDropReason` is emitted), and the unit suite was blind to it because every contribution test with a period axis passes a change fact alongside — the live configuration was pinned nowhere. **FIXED:** the anchor is now taken from any fact of the measure that names a period (change first, then variance), pinned by two tests incl. the exact live shape, and sabotaged. Measured on the fixture: every member cue moved from the Grand Total row to the 2025-12 row. **OWNER DECISION, MADE AND IMPLEMENTED 2026-09-18:** a movement's EXPLANATION must never outrank the movement it explains. The score bands overlapped — `change = 0.60 + 0.25*pct` against `contribution = 0.50 + 0.20*explained` — so setting them equal gives pct = 0.40: for any movement under 40% a fully-explained contribution beat its own change fact, and because priority and rank weight are applied per MEASURE they cancel between a measure's own facts and could not rescue it. On the fixture Revenue fell 10.4%, its three breakdowns scored 0.70, its headline 0.626, and `MAX_FACTS` cut the headline while keeping all three explanations of it. Fixed by lifting both headline kinds above the contribution ceiling of 0.70 — `variance = 0.81 + 0.19*pct`, `change = 0.71 + 0.19*pct` — which keeps variance's pre-existing 0.10 lead over change and reorders nothing else. Pinned by `a_movement_always_outranks_its_own_explanation` (sweeping every magnitude 0-100% against every explained fraction) and `a_variance_still_leads_a_change_of_the_same_size`; restoring the old band fails the first. The journey is green: seven cues, "Revenue down 10%" back on the month's Grand Total. **STILL OPEN, one part.** RESIDUAL: when NO fact of a measure names a period, a member fact still falls back to the member-alone cell, which on a pivot that HAS a period axis is the grand total — the same wrong cell, still silent. The mapper has no notion of "this pivot has a period axis", so the honest options are to carry the period ON the contribution fact in Rust, or to drop with a reason rather than place. | `app/src/api/pivotCues.ts` (`lastLabelByMeasure`), `app/src/api/__tests__/pivotCues.test.ts`, `app/src-tauri/src/insights/model.rs` (`MAX_FACTS`, the score table), `app/e2e/journeys/insight-overlays-pivot.spec.ts:497-502` |
| **The BI error surface still swallows failures in four places, and the pivot surface has no channel to report one at all.** FILED 2026-09-18 by a six-surface sweep (`refresh`, filter pane, CUBE, scripts/MCP, insights, pivots) run after the "One aggregate query path" change made security refusals reachable on all of them. **Closed in that round** (the root causes, which is why the swallows existed): `bi_refresh_connection` returned an ERROR for the benign "no grid queries" case, which forced the Connections pane to catch-and-ignore EVERYTHING and made the BI pane cry "Refresh failed" on a healthy connection — it returns `Ok(vec![])` now, the pane collects every per-pivot failure instead of discarding it, and `summarizeRefresh` (`extensions/BusinessIntelligence/lib/refreshOutcome.ts`) makes the quiet "nothing to refresh" line reachable ONLY from a run with no errors; `CubeError` gained a typed `Refused` variant matched on `EngineError::ObjectLevelSecurityDenied` / `RowLevelSecurityNotEnforceable` / `SecurityRoleNotFound` (never on message text), so a refusal stops folding into `Ok(None)` and reaching the MCP and AI-chat tools as `"(no value)"`; and every server-audited capability gate records the REFUSED call (`cube_udf_*`, `script_bi_sql`, the notebook `model.*` provider, `bi_query`, and `net.fetch`'s own send/body/size failures, which recorded nothing at all — found by the tightened guard in `capabilityAuditClassification.test.ts`, which now requires each gate to record an outcome with a RUNTIME reason rather than only its fixed permission-denied string). **A third round (2026-09-18) closed items 1-3 of that list.** The pivot response types gained a TYPED `notices: Vec<PivotNotice>` (`{kind: refused|degraded, message}`, omitted from the wire when empty), filled at the two `update_bi_pivot_fields` branches that return a blank pivot as if the field change had succeeded, and surfaced by `@api/pivotNotices` — which lives in `@api` because the three callers that most often TRIGGER those branches (the Slicer filter bridge, the Insert Slicer dialog, the Controls pane filter bridge) reach `updateBiPivotFields` straight from `@api/backend` and used to discard the response, so a consumer inside the Pivot extension would have been invisible exactly there. Drill-through no longer retries a SECURITY refusal without the denied dimension columns (`is_security_refusal`, promoted to `pub(crate)`; the engine's detail gate checks `dimension_columns`, so the bare retry passes the very gate that just refused) and its double-click handler now toasts the error — both in one change, because the backend fix alone would have turned a narrow sheet into a dead double-click, which is verbatim the BUG-0096 failure mode. The legitimate retry (a dimension the model cannot join in one hop) now says so. And the "view as" role change calls the new `@api/dataAftermath` seam, promoted out of Distribution rather than copied, which refreshes pivots, forces the CUBE round-trip and refetches the grid: `bi_set_active_role` only stored the role, so **every** BI-derived cell kept the previous role's value. **Still open from the sweep:** (4) the filter-pane store and the CUBE formula builder still swallow a refused column-values call, and `slicerFilterBridge.ts` / `filterPaneFilterBridge.ts` still `console.warn` a hard `Err` from `update_bi_pivot_fields` and return false; (5) Insights `notes` give a security refusal and "the calendar was guessed" the same type and the same grey. **Deferred deliberately, with reasons:** the totals grain cap (`totals.rs` ~276) and per-grain failure (~305) need `query_bi_total_overrides` to stop returning a bare `Vec<TotalOverride>` and thread a notice ~250 lines up, and the cap fires on LAYOUT not failure so it needs a told-you-once rule the response-scoped design does not provide; the two `refresh_table` swallows (`pivot/commands.rs` ~6790, `headless.rs` ~215) would fire on EVERY call, because both call sites' "one-time cost per table per session" comment is WRONG — `is_stale` with `max_age` ZERO is `elapsed() > 0` for a cached entry and `None => true` for a source-bound table that never caches; and the design-query route has no pivot chrome to show a notice in. **Newly filed, each its own decision:** the ENTIRE pivot module records nothing in the audit trail (`grep -rn audit app/src-tauri/src/pivot/*.rs` is empty), so a drill-through writes raw fact rows under an active security role and leaves no record, successful or refused — while net.fetch, bi.query, bi.sql and cube.value all now record refusals; `bi_set_active_role` emits no event, so the fix above is complete only while `BiPane` remains its single (MAIN-window-guarded) caller; and a DURABLE notice would live in `PivotState`, i.e. saved state, i.e. a `Persisted<T>`/`DocumentEffect` and probably a manifest-feature-id decision — not one to take inside a UI change. **Correction to this row's own earlier claim** that the cube case was "the only place a user reads a WRONG NUMBER": it is not. Inserted BI query blocks are rewritten only by `bi_refresh_connection`, which a role change never called either (now fixed by the same seam), and they have no `#N/A` fallback at all; and the totals grain cap silently rolls subtotals up from visible cells the moment a layout crosses 24 grains. Also: the cube case was worse than stated — the session latch is armed ONLY by TYPING a cube formula, so in a session where the user merely OPENED the workbook not even F9 would have refreshed those cells.  | `app/extensions/BusinessIntelligence/lib/refreshOutcome.ts`, `app/src/api/pivotNotices.ts`, `app/src/api/dataAftermath.ts`, `app/src-tauri/src/pivot/types.rs` (`PivotNotice`), `app/src-tauri/src/bi/cube.rs` (`is_security_refusal`, `audit_cube_call`), `app/src-tauri/src/pivot/commands.rs`, `app/extensions/ControlsPane/lib/filterPaneStore.ts`, `app/extensions/Slicer/lib/slicerFilterBridge.ts`, `app/src/api/scriptHost/__tests__/capabilityAuditClassification.test.ts` |
...
| **Four chart furniture elements are hit-testable and NAMED but cannot be selected, painted or formatted. FILED 2026-09-21 with the chart-interaction waves (CI-12, partial).** `trendline`, `errorBars`, `dataLabel` and `dataTable` are full members of `CHART_ELEMENT_IDS` (`app/extensions/Charts/types.ts:1644-1661`) with real producers — `elementHitTest-drift.test.ts:140` RUNS the hit-tester and would fail if they had none — but nothing downstream carries them. **(1) The ladder collapses them to chart level:** `advanceSelection` (`app/extensions/Charts/handlers/selectionHandler.ts`) handles axes, legend/legendEntry, the three titles and `datum`, and every other element falls into `if (element !== "datum") { subSelection = { level: "chart" }; }` in its final non-datum branch. **(2) Nothing paints them:** `CHART_SELECTABLE_ELEMENT_IDS` (`app/extensions/Charts/rendering/selectionHighlight.ts:247-254`) is exactly the six the ladder can reach, deliberately — painting an element nothing can select is the same defect in the other direction. **(3) The Format pane has no sections for them:** its subject switch (`app/extensions/Charts/components/ChartFormatPane.tsx:1816-1849`) ends in `default: return null`. **Two of the four could not even be NAMED by a selection if they had a route:** `ChartSubSelection` (`types.ts:1737-1747`) carries `seriesIndex`, `categoryIndex`, `axisType` and `elementId` and **no `trendlineIndex`** — while a hit already answers one (`ChartHitResult.trendlineIndex`, `types.ts:1699`), because one series can carry a linear fit and a moving average at once and `seriesIndex` alone cannot tell them apart — and no per-element `pointIndex` for a data LABEL, which is per point. A trendline is also recorded as a POLYLINE rather than a rect, so it needs a line painter of its own rather than `drawElementSelectionHighlight`'s box. The four therefore land together with a widened `ChartSubSelection`, not one at a time. | `app/extensions/Charts/types.ts:1644`, `:1699`, `:1737`; `handlers/selectionHandler.ts` (`advanceSelection`); `rendering/selectionHighlight.ts:240-254`; `components/ChartFormatPane.tsx:1816-1849`; `docs/design/chart-interaction.md` §5 (CI-12) |
| **`gridlines` is NOT in the chart element taxonomy, and the blocker is that nothing records where a gridline is drawn. FILED 2026-09-21 (CI-12, the deliberate half).** Excel addresses gridlines — all-or-none per axis per major/minor, never one line — so the omission reads as a gap. It is not: `drawHorizontalGridLines` / `drawVerticalGridLines` compute their ticks INSIDE the call and are invoked by twelve painters that never hand them a layout to write back to, so a `gridlines` member would be dead on arrival — the exact defect `CHART_ELEMENT_IDS` exists to prevent (`"title"` and `"legend"` sat in the previous union for a year with zero producers). It would not sit quietly either: `elementHitTest-drift.test.ts:140` runs the hit-tester and fails on a declared element nothing produces. The work is to thread a `ChartLayout` into those two painters and record the tick geometry the way `recordChartElementRect` does for the rest; the drift test then demands the producer on the day the member is added. Reasoning kept in the code at `types.ts:1633-1642`. | `app/extensions/Charts/types.ts:1633-1642`; `rendering/__tests__/elementHitTest-drift.test.ts:140`; `docs/design/chart-interaction.md` §6.6 |
| **`IKeybindingsAPI.register` does not expose the new `when` argument, so the facade and the function it fronts differ. FILED 2026-09-21.** `registerKeybinding(binding, when?)` (`app/src/api/keybindings.ts:811`) grew an applicability predicate — VS Code's when clause — because the shell's keybinding listener is capture-phase on `window` and installed before any extension activates, so an extension could not win Ctrl+1 from a listener of its own. The CONTRACT an extension sees on `ExtensionContext` still reads `register(binding: Omit<KeyBinding, "source">): () => void` (`app/src/api/keybindings.ts:74-78`), with no second parameter. Charts works only because it imports `registerKeybinding` directly rather than going through `context.keybindings` (`app/extensions/Charts/index.ts`, the `registerKeybinding` call for `ext.charts.formatSelection`), which is legal — it is still `@api` — but it means the documented facade cannot express a claim the module underneath it supports, and a 3rd-party extension reading the interface would conclude the feature does not exist. Widening it is one line plus a pass-through; the reason it was not done in the wave is that `IKeybindingsAPI` is part of `contract.ts` (`app/src/api/contract.ts:234`) and every extension context builder mirrors it. | `app/src/api/keybindings.ts:74-78` vs `:811`; `app/src/api/contract.ts:234`; `app/extensions/Charts/index.ts` (the `registerKeybinding` call for `ext.charts.formatSelection`) |
| **A per-point override cannot be REACHED on a default area chart, and cannot be honoured AT ALL by a custom mark. FILED 2026-09-21 (CI-2's two reachability gaps — coverage is complete, reach is not).** Every `builtin: true` mark honours `dataPointOverrides` and `rendering/__tests__/dataPointOverrideCoverage.test.ts` proves it per mark, in authoring space, under a filter. **Area:** `showMarkers` defaults to `false` (`app/extensions/Charts/rendering/areaChartPainter.ts:62`) and `resolveDatumStyle` is called only inside `if (showMarkers)` (`:291-308`); the area POLYGON is filled from the series colour alone (`:236-239`). So on a chart as inserted, colouring one point changes nothing visible until the reader turns markers on — and nothing in the UI says why. A per-point route for the polygon itself would need the fill split at the datum boundary, which is a painter change rather than a resolver change. **Custom / sandboxed marks:** `registerChartMark` is public, and `rendering/sandboxMarkShim.ts` runs the mark in a worker and blits an opaque `ImageBitmap` into the plot rect (`:97`) — there is no per-datum call to intercept, so this is a property of the sandbox boundary, not an omission. The coverage test scopes itself to `meta.builtin === true` and asserts a custom mark is EXCLUDED, with the reason, at `:273`. | `app/extensions/Charts/rendering/areaChartPainter.ts:62`, `:236-239`, `:291-308`; `rendering/sandboxMarkShim.ts:97`; `rendering/__tests__/dataPointOverrideCoverage.test.ts:62`, `:273` |
| **The Name Box renders an EMPTY input on first mount. PRE-EXISTING, found 2026-09-21 while wiring the chart selection readout (CI-9b).** `inputValue` is seeded to `""` (`app/src/shell/FormulaBar/NameBox.tsx:168`) while `prevDisplay` is seeded to the FIRST `displayValue` (`:399`), so the sync branch `if (displayValue !== prevDisplay)` (`:400-405`) — the only writer of `inputValue` outside editing — cannot fire on the first render. The box therefore shows nothing until something CHANGES the display value: the first cell click, the first chart selection. Not introduced by the chart work and not fixed by it: the chart label merely became one more producer of `displayValue` (`:395-396`). The fix is to seed `inputValue` from `displayValue` instead of from `""`, which needs a check that nothing depends on the empty first render. | `app/src/shell/FormulaBar/NameBox.tsx:168`, `:395-405` |
...
`Subject` on Dominance/Pareto would close it). The `model-engine-lib` finding that proof
surfaced — a derived measure (`Margin = [Revenue] - [Cost]`, the owner's `% Revenue of Total`)
with an EMPTY home table refused by the planner as `Table '' has no registered source` — is
FIXED (2026-09-17, BUG-0119; the GVAR half on `query_auto_refresh` is BUG-0120; changelog entry
"Derived measures query on every load path"; the follow-ups it left are in §2.1). Small: the narrator writes
