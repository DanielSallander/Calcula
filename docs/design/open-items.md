...
| **The in-app AI chat is hard-locked to one vendor, and offers no model picker at all.** Four shallow pins: the endpoint is a const (`ai_chat.rs:39`); the credential target is the single fixed string `Calcula:aikey\|anthropic`, so only ONE vendor's key can be stored at a time (`:36`); auth is Anthropic's `x-api-key` + `anthropic-version` rather than `Authorization: Bearer` (`:181-184`); and `thinking` is gated on `claude-opus-4` (`:159`). The **structural** pin is the fifth: the Anthropic wire format is built in the EXTENSION — `input_schema` tools (`AIChat/lib/chatTools.ts:29`), `stop_reason === "tool_use"` and `tool_use_id` result blocks (`ChatView.tsx:105-125`) — putting one vendor's JSON schema inside an extension, against the Facade Rule. `ai_chat.rs`'s own header states this as deliberate ("the frontend speaks the Anthropic wire format, so the Tauri layer stays thin"), so it is a decision to revisit, not an oversight to patch. **And the model is not selectable even within Anthropic:** `ai_chat_complete` accepts a `model` parameter, the caller never passes it (`ChatView.tsx:105-109`), so every request is `DEFAULT_MODEL = "claude-opus-4-8"` (`ai_chat.rs:38`), and the setup UI hardcodes "Connect Claude" with an `sk-ant-…` placeholder (`ChatView.tsx:147,151`). Owner intent (2026-08-19) is VS-Code-Copilot-style free choice of **any** model — local or cloud, any vendor. One `openai_compat` provider covers every local runtime *and* most cloud vendors; Anthropic keeps a native impl. Design: `docs/design/local-model-script-authoring.md` §3c, §7a, M3. | `ai_chat.rs:36,38,39,159,181-184`, `AIChat/lib/chatTools.ts:29`, `ChatView.tsx:19,105-125,147,151` |
