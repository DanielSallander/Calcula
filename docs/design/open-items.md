...
reproduction live in `tests/regression/bug-ledger.json` (**99 entries, 98 fixed, 1 open** as of
2026-08-17). The one open is **BUG-0098**, the backend wedge in §2.5, which is unreproduced — every
other ledgered defect is now closed. BUG-0095, BUG-0096 and BUG-0097 were fixed 2026-08-17; BUG-0099,
...
| **Two functional tests fail on a CLEAN app, and neither is attributed.** Found during the 2026-08-18 re-record and confirmed against a freshly launched instance, so they are not residue: `macro-live-edit.spec.ts:840` (a linked button runs the live-edited macro — the output cell stays `""` against an expected `"64321"` for the full 60 s poll, the button-click-does-nothing class) and `vba-idioms-wave3.spec.ts:468` (`setRangeFormat borderOutline` — the RIGHT edge of X63 samples **exactly 0** red pixels against a >0.15 threshold, while the TOP edge of the same outline passes). Three FURTHER failures seen alongside them (`vba-idioms-wave3` tests 2, 4 and 5) were state residue and pass on a clean app — worth knowing, because it means a dirty instance inflates this signal by 150%. **Ruled out as causes:** the dpr 2 -> 1 corpus move (every `samplePatch` derives its scale as `canvas.width / rect.width` from the canvas itself, so it is dpr-aware), and the harness changes of that session (all E2E/docs). Neither test owns a golden, so the corpus is unaffected either way. **Not yet established:** whether they passed at dpr 2 — this display cannot host the corpus at dpr 2 (see the `evaluate-formula` row above), so that comparison is not available here. The end-of-test screenshot is NOT evidence for either: the enclosing `finally` clears the range before Playwright captures, so both artefacts show an empty grid. | `e2e/tests/macro-live-edit.spec.ts:840`, `e2e/tests/vba-idioms-wave3.spec.ts:468` |
