...
| **Table transformations ship without multi-table steps — `mergeTable` (join) and `appendTable` (union).** Deliberately deferred, not forgotten: the step evaluator already takes a batch plus a schema, so a `TransformTableProvider` reading other tables' cached batches slots straight in. What it needs BEYOND that is refresh **ordering** — a topological sort over the table dependency graph with cycle rejection in `validate()` — which is the actual work and does not block anything shipped. Until then, a user who needs a join does it in a SQL-source import (`source_query`, now persisted) or a calculated table. | `docs/design/table-transformations.md` §"What is deliberately NOT built", `engine-core/src/transform/eval/mod.rs` |
