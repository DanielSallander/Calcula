...
| **The BI error surface still swallows failures in four places, and the pivot surface has no channel to report one at all.** FILED 2026-09-18 by a six-surface sweep (`refresh`, filter pane, CUBE, scripts/MCP, insights, pivots) run after the "One aggregate query path" change made security refusals reachable on all of them. **Closed in that round** (the root causes, which is why the swallows existed): `bi_refresh_connection` returned an ERROR for the benign "no grid queries" case, which forced the Connections pane to catch-and-ignore EVERYTHING and made the BI pane cry "Refresh failed" on a healthy connection — it returns `Ok(vec![])` now, the pane collects every per-pivot failure instead of discarding it, and `summarizeRefresh` (`extensions/BusinessIntelligence/lib/refreshOutcome.ts`) makes the quiet "nothing to refresh" line reachable ONLY from a run with no errors; `CubeError` gained a typed `Refused` variant matched on `EngineError::ObjectLevelSecurityDenied` / `RowLevelSecurityNotEnforceable` / `SecurityRoleNotFound` (never on message text), so a refusal stops folding into `Ok(None)` and reaching the MCP and AI-chat tools as `"(no value)"`; and every server-audited capability gate records the REFUSED call (`cube_udf_*`, `script_bi_sql`, the notebook `model.*` provider, `bi_query`, and `net.fetch`'s own send/body/size failures, which recorded nothing at all — found by the tightened guard in `capabilityAuditClassification.test.ts`, which now requires each gate to record an outcome with a RUNTIME reason rather than only its fixed permission-denied string). **Still open, in the order the sweep's critic recommended:** (1) **pivots have no notice channel** — `PivotViewResponse` / `DrillThroughResponse` carry no way to say anything, so five degradations (a capped totals grain, a failed `refresh_table`, a dimension-only pivot whose synthetic measure is denied) have nowhere to be reported even by code that wants to; add `notices: Vec<String>` and use it at the five producers; (2) **drill-through silently retries without the denied dimension columns** (`pivot/commands.rs` ~5133) — the highest-leak surface, and its TS catch (`extensions/Pivot/index.ts` ~1787) must be fixed in the SAME commit or the corrected error becomes a double-click that does nothing; (3) **switching "view as" never recalculates grid CUBE cells**, so every `CUBEVALUE` keeps the value computed under the previous role — the only place in the sweep where the user reads a WRONG NUMBER rather than a blank; (4) the filter-pane store and the CUBE formula builder still swallow a refused column-values call (console.error / a poisoned empty cache that never retries), and the canvas slicer's `slicerFilterBridge.ts` is an unreported twin of the ribbon one; (5) Insights `notes` give a security refusal and "the calendar was guessed" the same type and the same 11px grey under a reassuring empty state. **The rule to apply at every one of them**, which three sites could not express: *zero candidates enumerated* is quiet and informational; *zero successes out of N* is loud and an error — a "nothing to do" line must be reachable only from an enumeration that actually succeeded. **And match typed, never by substring:** this repo has been bitten three times now (the two dead RLS branches in `friendly_bi_query_error`, and a `message.includes("No active queries")` that shipped in the change above and was removed here). | `app/extensions/BusinessIntelligence/lib/refreshOutcome.ts`, `app/src-tauri/src/bi/cube.rs` (`is_security_refusal`, `audit_cube_call`), `app/src-tauri/src/pivot/commands.rs`, `app/extensions/ControlsPane/lib/filterPaneStore.ts`, `app/extensions/Slicer/lib/slicerFilterBridge.ts`, `app/src/api/scriptHost/__tests__/capabilityAuditClassification.test.ts` |
