...
- **Behavior change (fix).** `query_auto_refresh` runs the same pre-planning chain as `query` (ISFILTERED fold → GVAR resolution → calculation-group expansion, one shared helper), so GVAR and ISFILTERED measures evaluate there. It still rejects multiple active roles and still does not apply object-level security, `rank_by`, `top_n` or `measure_filters` — those remain `query`-path only (tracked in Calcula's `docs/design/open-items.md`). `query_auto_tier` is unchanged and still fails closed on GVAR.
