...
| `15` | Materialized calculated tables — `GlobalVariable` gained the dynamic flag / calendar spec; derived tables carry `is_calculated`; `QUERY(DISTINCT ...)`. |
| `16` | `IsFiltered` expression variant (`ISFILTERED` direct-filter check). |
| `17` | `LookupValue` variant (`LOOKUPVALUE` in calculated columns) + cross-table calculated-column references. |
| `18` | Measure `format_string_expression` (dynamic format strings). |
| `19` | DAX-gap batch — `PATH` calculated columns + `PATHLENGTH`/`PATHITEM`, measure `detail_rows`, security-role OLS denials, perspectives. |
| `20` | Second DAX-gap batch — `THISROW` anchor rows, `DATESBETWEEN`, Week granularity/`WTD`, `fiscal_year_end_month`, cultures/translations. |
...
## One aggregate query path: auto-refresh, auto-tier and explain enforce object-level security, honour `rank_by` / `top_n` / `measure_filters`, and run the multi-role union (no `MODEL_FORMAT_VERSION` change)

`query_auto_refresh` — the path behind Calcula's `bi_query` (script capability, notebook `model.query`, trusted calls), filter-pane column values, BI Refresh, CUBE formulas, Insights and MCP `run_bi_query` — planned on its own: it applied **no object-level security**, silently **dropped** `rank_by` / `top_n` / `measure_filters`, refused multiple roles instead of running the union, skipped context-column pushdown and ran a non-cancellable executor. `query_explained` (the Model Editor's plan preview) did the same minus the refresh, so toggling "include plan" changed the RESULT ROWS and bypassed OLS under a preview role. `query_auto_tier` (no host caller) additionally skipped GVAR/ISFILTERED resolution (fail-closed) and — the only real hole — ran a multi-role request with EMPTY role filters. A pivot on the same connection refused all of it through `query`. Filed as the open item behind BUG-0120's "deliberately unchanged" note; the trail continues here.

- **Behavior change (security).** All three paths now run the one aggregate core. `query_auto_refresh` is literally the validations + `refresh_stale` + `query_with_cancellation`; `query_auto_tier` is the validations + tiering + TTL refresh + `query_with_cancellation`; `query_explained` runs the same gates and rewrites ahead of the explained plan. Object-level security is therefore enforced on every aggregate entry point: a request touching a denied table or column fails with `QueryError::Engine(EngineError::ObjectLevelSecurityDenied { object })` (Display: `object-level security: the active role denies access to Category[name]`). **Host impact:** that refusal now reaches `bi_query` (scripts, notebook, MCP), the filter pane, Refresh, CUBE cells (`#N/A`), Insights (a report note) and the Model Editor's plan preview under a preview role — exactly where a pivot already refused.
- **Behavior change (security).** The OLS gate now also inspects `scoped_filters` / `scoped_in_filters` — the shape every host filter arrives in — on EVERY path, `query` included: a table-qualified scoped filter on a denied column is refused like a group-by column; an unqualified one falls back to the conservative bare-name match. Previously a caller could bisect a denied column's values through `WHERE Employees[Salary] > x`.
- **Behavior change.** `rank_by`, `top_n` and `measure_filters` are honoured on `query_auto_refresh`, `query_auto_tier` and `query_explained` (they were silently dropped there), with the same validation and precedence as `query` (rank outermost, then top-N, then HAVING; the inner query runs with no row limit and `limit` is applied once, after the outermost step). One shared peel (`peel_post_aggregation` / `apply_post_aggregation`) replaced the three recursive helpers.
- **Explain contract.** When a step is peeled, `query_explained` returns the INNER query's plan plus one more child of the root: `PlanOperation::PostAggregation` (new enum variant) with properties `step` (the peeled option, Debug-rendered), `rows_in`, `rows_out` and — when the request carried one — `limit`, and its duration added to the root and to `total_duration`. A host that renders operations by name needs a fallback for the new name (Calcula's viewer has one and a `POST` badge). A request without a step reports no such node, so existing plan shapes are unchanged.
- **Behavior change (security).** The multi-role union runs on every aggregate path; `reject_multi_role` guards only drillthrough (`query_rows`), whose message now reads "… drillthrough requires a single active role". The union's unsupported shapes fail closed identically everywhere (see the RLS entry below), and `GVAR` under multiple roles is still refused on all four paths. The pre-checks (UDFs, role, OLS, and the post-aggregation step's shape) run BEFORE the refresh / tiering step, so a request one of them refuses triggers no source fetch and no cache write; only the union's own shape refusals (cross-table roles, a multi-predicate role — the builder plans to prove enforceability) still follow the refresh. The union is also order-independent now: every role's predicates are resolved before any shape check, so `[A, B]` and `[B, A]` give the same answer — an unrestricted role in the set means no restriction, whatever the other roles' shapes (`security_tests::multi_role_union_is_order_independent`; the four-path GVAR refusal is `gvar_tests::gvar_with_multiple_roles_fails_closed_on_every_aggregate_path`).
- **Behavior change.** `query_auto_tier` resolves GVAR and ISFILTERED measures (it used to fail closed; `gvar_tests::gvar_via_auto_tier_fails_closed` is inverted to `gvar_resolves_on_auto_tier_path`, and `derived_measure_tests::a_derived_measure_carrying_isfiltered_queries` now covers the auto-tier path too) and honours calculation groups as before.
- **Behavior change (security, all paths).** The `ISFILTERED` fold now reads the request as the caller wrote it, BEFORE the multi-role union injects the roles' predicates as `or_filters`: a role predicate is sealed security, not a user filter, so `ISFILTERED(Geography[region])` folds the same way under two roles on that column as under one (it used to fold TRUE under the union with no user slicer, so a measure gating disclosure on `ISFILTERED` disclosed under two roles and blanked under one — pre-existing on `query`, now reachable on every path, so fixed here; pinned by `security_tests::isfiltered_ignores_the_multi_role_unions_injected_predicates`).
- **Behavior change (library-only today).** The auto-tiered set is kept truthful: `drop_table_cache` / `drop_table_cache_and_dependents` remove the dropped table from it and `set_model` retains only tables the new model still declares as non-in-memory. Without this a tiered table whose rows were dropped stayed "local" for every planner call (passing, for one, the filter-context time-intelligence gate) while the executor fetched it from the source with the request's date filter — the shifted-into-nothing answer that gate exists to refuse (pinned by `auto_tier::tests::a_dropped_tiered_table_leaves_the_planners_local_set`).
- **Behavior change (freshness; library-only today).** Every planner call — `plan_and_execute`, the multi-role enforceability probe and `query_explained` — is given the auto-tiered table set, so once `query_auto_tier` has tiered a dimension, a plain `&self` `query()` plans it as local too (served from the tier cache, which only `query_auto_tier` TTL-refreshes) where a single-source query previously pushed to the source. Calcula sets `AutoTierConfig` but never calls `query_auto_tier` / `auto_tier_remaining`, so its tiered set is always empty and its plans are byte-identical. Pinned through the planner's filter-context time-intelligence gate (which refuses unless the date table is in-memory or in the set): `auto_tier::tests::a_tiered_dimension_is_local_on_the_plain_query_path` — a tiered DirectQuery date table plans on `query`, `query_explained` and the multi-role union probe, and refuses when nothing is tiered.
- **Cache contract (pinned).** `query_cache_key` hashes none of `rank_by` / `top_n` / `measure_filters`; only the INNER request's rows are ever stored, so a ranked result is never served to the plain request or vice versa (`rank_tests::a_peeled_result_is_never_cached_under_the_plain_request_key`).
- **New public API.** `Engine::check_object_level_security(&self, &QueryRequest) -> QueryResult<()>` — the OLS gate as a pure check (no I/O, no cache write), for a host that must choose a request the active role permits inside the lock it queries under. It validates the active role first, so an unknown role is `SecurityRoleNotFound` here exactly as it would be on a query rather than a false "permitted". `PushdownPlanner::plan_explained_with_cached(request, model, registry, cached_tables, role_filters, context_column_cases)` — `plan_explained` with the engine's cached-table set; `plan_explained` delegates with an empty set. `PlanOperation::PostAggregation`.
- **Calcula action (done in the same change).** The filter pane's two column-values commands used `measures().first()` as their placeholder measure; the OLS gate walks a requested measure's whole closure, so a first measure touching a denied column would have refused every slicer on the connection. They now take the first measure the role permits (`distinct_values_request`, probed with `check_object_level_security` INSIDE the lock after `apply_connection_role`, declared order kept because the placeholder's fact table decides which members appear); when nothing passes the error names the column if the column is what is denied. `friendly_bi_query_error`'s two RLS branches matched Rust variant names against Display text and were dead — fixed, plus a "View as" hint for OLS refusals, pinned by `error_mapping_tests`. CUBE cells keep `#N/A` (a cell has nowhere to put a message, and that is Excel's semantics), but a refusal is now a DISTINCT `CubeError::Refused` matched on the typed engine error, so the surfaces that *can* carry a message no longer report it as success: a script's `cube.value` returns an error instead of `Ok(None)`, the MCP and AI-chat `cube_value` / `cube_kpi` tools stop rendering it as `"(no value)"`, and every server-audited capability gate (`cube_udf_*`, `script_bi_sql`, the notebook's `model.*` provider, and `net.fetch`'s own outcome) records the refused call as a failure instead of leaving the trail showing a success or nothing at all. The Model Editor's "include plan" preview now returns the same rows as without it. `bi_refresh_connection` now checks every active query's objects with `check_object_level_security` in ONE lock before it rewrites any grid block, and a query that fails later (a source error, say) stops the loop without skipping the dependents recalculation — it used to leave earlier blocks rewritten with their formulas holding the previous refresh's numbers. A `bi.query` refused by security is written to the script-activity audit trail (the reason is the engine's message; object names, never row data), not only a refusal of the capability itself. The Connections pane reported every grid-query refresh failure as "No queries or pivot tables to refresh" — it now distinguishes the benign "no active queries" case from a real refusal and shows the message. Known and accepted: the filter-pane store and the CUBE formula builder still swallow a refused column-values call (console / empty list) — only the pinned-slicer route surfaces the message.

---

## Derived measures query on every load path; `query_auto_refresh` runs the pre-planning overlays (no `MODEL_FORMAT_VERSION` change)

A measure whose leaves are all `[Measure]` references (`[Revenue] - [Cost]`, `DIVIDE([Margin], [Revenue])`, `GVAR total = [Revenue] RETURN …`) carried an **empty home table** unless the model came through `Engine::load_model`; every other way a model reaches an engine — `Engine::new`, `with_memory_budget`, `set_model`, i.e. every Calcula load path (`create_connection_core`, the `.cala` restore, the `.calp` subscribe and refresh) — left it empty, and the pushdown planner refused the query with `QueryError::SourceNotRegistered("")`, an error naming no table. Where the table HAD been resolved (a Model Editor edit), a pushed-join plan still handed the raw reference to the connector and failed with "Measure reference '[Revenue]' must be expanded". Separately, `query_auto_refresh` — the path behind `bi_query`, column values, refresh, insights, cube and MCP — skipped the ISFILTERED fold and the query-scoped (`GVAR`) resolution that `query` / `query_explained` run, so a GVAR measure failed there ("reached the executor with an unresolved query-scoped (GVAR) binding") while the same measure worked in a pivot. Found live by Calcula's insights route on its sales-star fixture (`Margin`, `MarginPct`) and on an AdventureWorks model (`% Revenue of Total`).

- **Behavior change (fix).** The planner now resolves every requested measure against the model (references inlined, home table re-inferred, host-facing metadata kept) before reading a table name, so a derived measure plans and executes exactly like its column-form equivalent — plain, under a calculation-group application, carrying an `ISFILTERED` marker, and on the pushed-join path — on `query` / `query_with_cancellation` / `query_with_meta` / `query_explained` / `query_auto_refresh` / `query_auto_tier`, whatever built the model. A derived measure that is a pure alias (`[Revenue]`) can now reach a pushed single-table plan; the SQL is the one its column form already produced. Cross-fact measures keep the first-wins rule (`[Revenue] - [Budget]` is homed on Revenue's table), now pinned as a deliberate limitation.
- **Behavior change (fix).** `Engine::new`, `with_memory_budget` and `set_model` call `DataModel::resolve_measure_home_tables` (fill-only, idempotent): `engine.model().measures()[i].table()` is the home table of the measures a derived measure builds on. A constant measure still reports `""`. `DataModel::builder().build()` and `validate()` are unchanged. **Host impact:** `MeasureFieldInfo.table` in the pivot field list is now populated for derived measures, so the field-list OLS filter applies to them: a derived measure whose HOME table is denied is hidden like the measures it builds on. (The home table is the first-wins one; a cross-fact derived measure whose other table is denied stays listed and is refused by the engine's query-time gate, which walks the whole closure — the same split a column-form measure over two tables already has.) `bi_model_get_measures` reads the host's own `base_model` clone, which is taken BEFORE the engine is built, so the Model Editor's listing still shows `""` for a derived measure until it is edited; the pivot field list reads `engine.model()`.
- **Error change.** A request naming a measure whose resolved closure references no table (`BLANK()`, `42`) now fails at plan time with `QueryError::InvalidQuery("measure 'X' references no table … constant measures cannot be evaluated by a query in this version")` instead of `SourceNotRegistered("")`. No such query has ever succeeded.
- **Behavior change (fix).** `query_auto_refresh` runs the same pre-planning chain as `query` (ISFILTERED fold → GVAR resolution → calculation-group expansion, one shared helper), so GVAR and ISFILTERED measures evaluate there. *(As of this entry it still rejected multiple active roles and did not apply object-level security, `rank_by`, `top_n` or `measure_filters`, and `query_auto_tier` still failed closed on GVAR — all closed by the entry above: the path is now `refresh_stale` + `query_with_cancellation`.)*
- **New public API.** `Measure::resolved_against(&self, &DataModel) -> EngineResult<Measure>` — the one sanctioned way to get a trustworthy `table()` and a reference-free expression from a stored measure. `Measure::table`'s doc now says when a derived measure's table is filled.
- **Walker consistency (no observable change).** `has_measure_ref` now also sees a `[Measure]` inside a `KEEP` condition, as `expand_measure_refs` and `Expression::measure_references` already did, so the three walkers agree. A measure of that shape (`SUM(Sales[amount], KEEP(Sales, Sales[amount] > [AvgAmount]))`) still cannot be evaluated — the reference expands to an aggregate inside a row-level predicate — it now fails at DataFusion rather than at the SQL renderer.
- **Calcula action (done in the same change).** The engine resolves the copy it is GIVEN; a host that keeps its own copy of a deserialized model must resolve that copy itself. Calcula's `base_model` clone is taken before the engine is built and is what the Model Editor lists, what `insights_analyze_model` builds its facts from and what every later `set_model` starts from, so `DataModel::resolve_measure_home_tables()` is now called right after `serde_json::from_value` at the four host deserialize sites (`bi/commands.rs` create + restore, `calp_commands.rs` subscribe + refresh) and in the read-only strategy validator. The existing calls in `bi/measures.rs` and `bi/model_editor.rs` stay (they feed the "must reference at least one column" message). Measured before this: the insights notes on the sales-star model read "Product is not directly related to " with an empty table name for `Margin` — the host's copy, not the engine's.
...
## Filter levels — pinned filters and `LEVEL n` on the clear family (format version 27)

Every filter that reaches a measure now sits on a **level**: 0 = the group-by axis, 1 = ordinary slicers/query filters (the default), 2..=9 = **pinned** filters. Each clear op removes a contiguous level range — `CLEAR_INNER` is `[0,0]`, bare `CLEAR`/`RESET` are `[0,1]`, `CLEAR_OUTER`/`RESET_OUTER` are `[1,1]` — and an optional trailing `LEVEL n` argument raises the ceiling (`CLEAR(dim_customer, LEVEL 2)` clears `[0,2]`). The practical payoff: a filter pinned at level 2 sits **out of range** of every bare `CLEAR`, `RESET`, and `CLEAREXCEPT` in the model, so "always keep this filter" no longer needs per-measure `CLEAREXCEPT` boilerplate — pin it once at the request layer.

- **Expression syntax:** `LEVEL n` (n = 0..9) as the LAST argument of `CLEAR` / `CLEAR_OUTER` / `RESET` / `RESET_OUTER`. `LEVEL` is a *contextual* keyword — claimed only when followed by a number, so a table literally named `level` still parses as a target. `LEVEL 0` canonicalizes to the `_INNER` spelling and `LEVEL 1` to the bare form (a saved formula renders back canonically). `CLEAR_INNER`/`RESET_INNER`/`ALLSELECTED` refuse `LEVEL` with a hint; the `_OUTER` forms refuse `LEVEL 0`; out-of-range or fractional levels are parse errors. The same grammar works inside CONTEXT definitions.
- **Model JSON:** `Expression::{Clear, Reset, ClearOuter, ResetOuter}` gained *additive* `level: Option<u8>`; `ContextOp::{Clear, ClearOuter, Reset, ResetOuter}` became **struct variants** carrying `targets`/`level` (`{"Clear": {"targets": […]}}` — a deliberate pre-production shape break for CONTEXT definitions). `MODEL_FORMAT_VERSION` **26 → 27**: the additive field means a pre-v27 engine would load a leveled measure and silently treat `CLEAR(dim, LEVEL 2)` as `CLEAR(dim)` — a misinterpretation, so the gate refuses instead. A level above 9 arriving from hand-edited JSON is refused at context resolution (fail closed).
- **Request contract:** `QueryRequest` gained `scoped_filters: Vec<ScopedFilter { table: Option<String>, condition: FilterCondition, level: u8 }>` and `scoped_in_filters: Vec<ScopedInFilter { table, filter: InFilter, level }>` — level-tagged, table-attributed counterparts of `filters`/`in_filters` (which stay level-1, column-ownership-resolved). Scoped filters are validated strictly: unknown table/column, level 0, or level > 9 refuse rather than silently widening the result. Requests carrying scoped filters take the LocalAggregation path. Both new lists are hashed into the query cache key.
- **Behavioral change — slicer removal now EXECUTES (was: always fail closed).** A measure whose clear range covers a request filter's level makes that filter **contested**: the planner withholds it from every fetch WHERE and the local executor applies it *per measure* — the clearing measure evaluates without it (true REMOVEFILTERS), every other measure gets it as a conditional-aggregation condition, and the result's rows still honor every filter at every level (groups need ≥ 1 row matching all contested filters). Supported for plain aggregate measures on the single-fact local path, including scalar (no group-by) requests and identity-partition clears. `plan_explained` reports each contested filter.
- **Still fails closed, with precise errors:** contested + compound measures (percent-of-total shapes — their sub-expression contexts resolve independently), contested + an axis-spanning window re-aggregation (a grouped broadcast total over a sliced table would return the *visible* total, not the cleared one), contested OR slicers, contested + ROLLUP, the window/TI, multi-fact, pre-aggregate, QUERY-in-VAR, and split-override paths, ambiguous column ownership, and non-safe-join dimensions. Pinned filters need none of this machinery — they are simply out of clear range, so pinned queries run everywhere today's queries run.
- **RLS is outside the level model entirely:** role predicates are sealed into fetches and never enter context resolution — `RESET(LEVEL 9)` cannot recover role-excluded rows (pinned by test).
- **Pushed-path parity fix:** the PostgreSQL CLEAR/CLEAREXCEPT window render previously hard-coded `SUM` as the re-aggregation regardless of the wrapped aggregate (a `MIN` under CLEAR silently summed). It now mirrors the local `clear_reagg_fn` — SUM/COUNT/COUNTROWS re-sum, MIN/MAX carry, everything else fails closed.
- **Display/DTO:** `ContextOp::to_text()` and the measure formatter render `, LEVEL n` (canonical bare forms omit it). The host's `ContextOpDto` gained `level?: number`.
- **Calcula action.** Send slicer/filter levels via `scoped_filters`/`scoped_in_filters` (or `BiFilter.level`); default 1, pin at 2+. Surface the pin level in filter UI (a pinned filter changes what a CLEAR measure returns — make it visible). Treat the new refusal messages as user-facing guidance; they name the workaround. Stamp `format_version >= 27` when a model carries a leveled clear.
...
- **Setting steps makes a table `InMemory`.** `with_table_transformations` (and therefore `Engine::set_table_transformations`) stamps it, because model validation refuses any other mode for a transformed table — so it is the only mode the edit could have succeeded with, and making the caller discover that through a validation error makes a legal edit look like a mistake. Clearing the steps does **not** switch back; by then the mode is the user's to keep. Hosts importing REST endpoints must stamp `InMemory` themselves (the engine refuses a DirectQuery table bound to a REST source).
...
- **Preview is a sample, not the answer.** `preview_transformations` fetches at most `MAX_PREVIEW_ROWS` (10 000) source rows and returns a `TransformPreview { batch, source_rows, source_truncated }`. The batch is **conformed to the schema the previewed steps derive**, exactly as a refresh conforms to the table's declared columns, so a preview cannot render a value the refresh would store differently. The source counts are returned rather than left to the caller **because a caller cannot infer them**: a pipeline that filters rows out leaves an output far under the cap even when the source was truncated, so judging "is this a sample?" from the output row count under-reports exactly when an aggregate is most likely to mislead. Combine `source_truncated` with `TransformStep::changes_row_count()` to decide whether to label a preview indicative rather than final.
- **`changeType` refuses to round.** Arrow's non-safe cast converts fractional values to integers by TRUNCATION rather than failing, so the `Fail` policy now round-trips the cast and refuses when any value would change; the `Null` policy blanks exactly those values. A lossless float-to-integer cast still succeeds.
- **`pivot` counts per value.** A pivot with `AggregateOp::CountRows` renders `SUM(CASE WHEN … THEN 1 END)`, not a bare `COUNT(*)` — the latter ignores its operand and gave every pivoted column the whole group's row count.
- **Aggregates that derive `Float64` compute in `Float64`.** `groupBy`/`pivot` cast the operand to `DOUBLE` when derivation declares a fractional result over a non-fractional column, so `median` over an integer column no longer computes an integer-truncated median (and `SUM` over `Decimal` no longer returns a widened decimal the conform step then re-casts).
- **`replaceValues` renders typed literals.** Whole-value mode parses the author's text as a value of the column's type and re-renders it; text that is not such a value is refused at model-build time, naming the column. Nothing from a model file reaches a generated statement unparsed.
- **Re-binding a table keeps its pipeline.** `bind_source_table` / `bind_source_tables` preserve `transformations`, `source_columns` and `source_query` when re-pointing a table that already has them.
- **A `Link: rel="next"` URL must name the source's own host.** The REST connector re-validates the scheme AND pins the authority, because it attaches the resolved credential to every request — an unpinned continuation URL is the same attack `redirect::Policy::none()` refuses.
- **Error responses obey the size cap.** A non-2xx body is read chunk-by-chunk up to the snippet length instead of being buffered whole.
- **Calcula action.** Stamp `format_version >= 24` when a table carries `transformations`, `source_columns` or a persisted `source_query`, or when the model has a `rest` source. Drive the step editor with `derive_pipeline_schema` (live column list + per-step diagnostics, no I/O) and `preview_transformations` (data, cancellable). Commit with `set_table_transformations` — it is one model edit, so it batches into one undo entry. Migrate SQL-source imports to persist `source_query` on the binding rather than in host-side storage. Surface `InvalidTransform` / `TransformFailed` against the step their `step_index` names.
...
## Expression-language batch — calc-group introspection, NOT IN, day-level time intelligence, QUERY TOP (format version 23)

A batch of measure-language additions. All new model-JSON fields and AST variants are *additive* (serde defaults), and the `MODEL_FORMAT_VERSION` bump to **23** gates files that use them.

- **Calculation-group introspection (parser + AST).** Calculation items (and selection expressions) may now use `ISSELECTEDMEASURE([M1], [M2], ...)`, `SELECTEDMEASURENAME()`, and `SELECTEDMEASUREFORMATSTRING()`. All three are **folded at application time** (compare/substitute against the applied measure) and never reach planning; like `SELECTEDMEASURE()`, they are rejected in ordinary measures/calculated columns. `ISSELECTEDMEASURE` arguments are validated against the model at build and reported by `measure_references` / lineage.
- **Per-item dynamic format strings (model JSON, v23).** `CalculationItem` gained `format_string_expression` (source text; `CalculationItem::with_format_string_expression` / `format_string_expression()`), the item-level analog of the measure-level dynamic format string, typically built with `SELECTEDMEASUREFORMATSTRING()`. Evaluated once per query per transformed measure in `query_with_meta`; **format precedence for a calc-group column**: item expression → item static format → base measure's dynamic format → base measure's static format. (Also a behavior fix: an item's *static* format now wins over the base measure's dynamic format for that column.) Constant expressions are folded without a query via the new `const_fold_scalar` (`engine_core::compute::expression::{const_fold_scalar, ConstFold, FoldValue}`, public). Validated at model build (parse + no GVAR/QUERY/LOOKUPVALUE + measure-reference checks).
- **Public API change.** `Expression::substitute_selected_measure` now takes `&SelectedMeasureInfo { expression, name, format_string }` (new public struct) instead of `&Expression`, so the whole placeholder family folds in one pass. Hosts calling it directly must wrap the measure in the info struct.
- **`NOT IN` (expression syntax + AST fields, v23).** KEEP conditions and predicates accept anti-membership: `col NOT IN {literals}` (`InList` gained `negated: bool`) and `Table[col] NOT IN var[col]` (`InPredicate` gained `negated: bool`, threaded through `ResolvedInFilter.negated`). SQL `<>` semantics: BLANK satisfies neither form; an empty set keeps everything under `NOT IN`. Runs on the same paths as `IN` (local aggregation + MeasureEngine); unconsumable paths keep failing closed.
- **Day-level time intelligence (v23).** `DateGranularity` gained `Day`; `PRIORPERIOD`/`PARALLELPERIOD`/`DATESINPERIOD` accept a `DAY` interval (filter-context only — a date column on the axis fails closed, like `WEEK`). New functions `PREVIOUSDAY(x)` / `NEXTDAY(x)` (the single day adjacent to the context's first/last date) and `FIRSTNONBLANK(x)` / `LASTNONBLANK(x)` (the first/last context date **with fact data** — the inner must be a simple single-column aggregate or COUNTROWS; the boundary is probed **once per query**, not per group-by cell — documented divergence from DAX). All four ride `SemiAdditiveBalance`, which gained `shift_days: i64` and `non_blank: bool` (additive AST fields).
- **Value-based (gap-tolerant) period shifts — behavior change.** A filter-context `PRIORYEAR`/`PRIORPERIOD`/`PARALLELPERIOD` over a date context with an internal hole (e.g. Jan+Mar without Feb) previously **failed closed** ("not contiguous"); it now computes the **value-based** shift: every distinct context date is shifted individually (DAX `DATEADD` semantics including the end-of-month snap: a month-end date shifts to the target month's end) and the lowered filter keeps exactly the shifted set. Contiguous contexts keep the previous algebraic range lowering (identical results). Composes with ROLLUP/hierarchy like before. Cap: > 20 000 distinct context dates fails closed with a typed `TimeIntelligence` error. The axis (positional) path keeps its contiguity guard unchanged.
- **`QUERY ... TOP n BY alias [ASC]` (AST field, v23).** `Expression::Query` gained `top: Option<QueryTop>` (`QueryTop { by, limit, ascending }`, public). A **tie-inclusive** top-N (all boundary ties kept, like the request-level `top_n`) applied to the materialized intermediate — per **outer group** when the query is re-evaluated per group. Enables top-N-set measures (`VAR t = QUERY(SUM(f[amt]) AS a BY p[cat] TOP 3 BY a) RETURN SUM(t[a])`) and top-N **calculated tables** (the materialized snapshot keeps the top rows). The ranked column must be one of the QUERY's outputs (parse/build error otherwise) and numeric (typed error).
- **Not shipped — `CROSSFILTER` (per-measure propagation override).** Deliberately deferred: relationship filter-propagation is applied at the shared **fetch** level, so a per-measure override cannot be honored without per-measure fetch isolation; a partial wiring would produce silently-wrong results on the shared paths (the same structural reason `USERELATIONSHIP` × time-intelligence fails closed). Model-level `Relationship::with_propagation(FilterPropagation::Both/None)` remains the supported control; per-measure de-propagation is expressible with `CLEAR(dim)`.
- **Errors the host can observe (new/changed).** Calc-item-only rejections now name the specific function (`ISSELECTEDMEASURE()` etc.); `QUERY TOP` shape errors are parse/build errors; `FIRSTNONBLANK`/`LASTNONBLANK` with a non-simple inner and the >20k gapped-shift cap surface as `EngineError::TimeIntelligence`; an unforeseeable non-constant format expression on a pure-literal path surfaces as `QueryError::InvalidQuery` (previously `SourceNotRegistered("")`).
- **New public inspectors (host stamping).** `Expression::contains_calc_group_introspection` / `contains_negated_membership` / `contains_day_level_time_intelligence` / `contains_query_top` — recursive AST checks covering the four expression-level v23 features (the same pattern as the v16–v20 `contains_*` inspectors). Item/selection `format_string_expression` is detected via its accessor; together these are exactly the v23 stamping conditions.
- **Studio / Calcula action.** Stamp `format_version >= 23` when a model uses any of: the three introspection functions, `format_string_expression` on a calculation item, `NOT IN`, `DAY` intervals / the four new time-intelligence functions, or `QUERY ... TOP`. Surface the new Model Editor affordances as desired (item format expressions; NOT IN in filter builders).
...
- **Not supported (v1, fails closed):** `GVAR` in a calculated column, model global variable, or **calculation item** (rejected at model build — applying a group always fails closed on GVARs, so an item declaring one could never run); evaluation through the lower-level in-memory `MeasureEngine`; combination with a calculation group; `GVAR` under **multiple active RLS roles** (`set_active_roles` with >1 role — query under a single role instead). *(Evaluation via `query_auto_tier` was also unsupported here — it failed closed with `QueryError::InvalidQuery` — until the "one aggregate query path" entry above; every aggregate entry point now resolves `GVAR`.)* Adding `GVAR` to a measure is backward-compatible for authoring: `GVAR` is only a keyword in full declaration position (`GVAR <name> =`), so existing measures that use `gvar` as a variable name — or that merely *begin* with a column/measure/global reference named `gvar` (e.g. `gvar + 1`) — still parse.

### GVAR hardening (post-release review; no format bump)

- **`Engine::validate_measure_text` now enforces the full GVAR rule set** (scalar-only binding — including a window/`QUERY` nested anywhere inside it — no `VAR`/forward references, duplicate names, top-level placement, and the GVAR-name vs model global-variable collision). Previously the editor-time API accepted some measure texts that the subsequent model build rejected; hosts now get the same verdict from both. **Behavior change:** text that previously validated OK but could never build now returns `EngineError::InvalidExpression` at validation time.
- **Constant date `GVAR` bindings now evaluate.** A binding with no table reference folds through the same machinery (and local clock) as incremental refresh filters: literals, arithmetic, `DIVIDE`, `COALESCE`, and the date functions `TODAY()` / `DATE(y,m,d)` / `DATEADD(...)` / `DATETRUNC(...)` — so `GVAR asof = DATEADD(TODAY(), -30, "DAY")` works, including chaining through earlier date GVARs. `NOW()` remains unsupported in a table-less binding. Non-constant table-less bindings get a clearer error message.
- **New public API `DataModel::replace_measure(&mut self, Measure) -> bool`** — in-place, case-insensitive-by-name measure replacement; the mutable sibling of `with_measures` (same caller-validates contract).
- Internal hardening with no host contract change: the pushed-SQL and conditional renderers fail closed on an unresolved GVAR; the executor's fail-closed guard now also catches a GVAR hidden behind a `[Measure]` reference on non-primary query paths; per-query GVAR resolution overhead was removed for models with no GVAR measures.
- **`Engine::query_explained` now resolves GVARs (behavior change).** The explain path previously skipped the facade GVAR-resolution step, so a GVAR measure failed closed with `QueryError::InvalidQuery` ("reached the executor with an unresolved query-scoped (GVAR) binding"). It now runs the same pre-planning overlays as `query` (ISFILTERED literal fold + GVAR resolution), so a query with an execution plan succeeds and reports the plan the query actually runs. `query_auto_refresh` now resolves them too (see the derived-measures entry above); `query_auto_tier` *(still failed closed on GVAR measures at the time; resolved since the "one aggregate query path" entry)*.
...
When two or more roles are active, every aggregate path (`query`/`query_with_cancellation`, and since the "one aggregate query path" entry also `query_auto_refresh`, `query_auto_tier` and `query_explained`) combines them with Power BI's union semantics. v1 supports the union when **every active role restricts the same table with exactly one predicate** — the engine rewrites the set into a sealed single-table `OR` slicer that rides the same enforceable single-hop propagation as a single role. Cross-role isolation is preserved: the canonicalized (order-independent) role set is part of the query-cache key.
...
Drillthrough (`query_rows`) does not implement the union: it **fails closed under multiple active roles** ("… drillthrough requires a single active role") rather than run with no restriction. Activate a single role for drillthrough. *(Until the "one aggregate query path" entry, `query_auto_refresh` and `query_explained` failed closed the same way.)*
