...
def main():
    ...
    # 9. GENERATE MAP (With Folding, Grouping, Compression & Log Highlighting)
    map_lines = []
    ...
    for directory in sorted(files_by_dir.keys()):
        ...
        for f in sorted(files_by_dir[directory], key=lambda x: x.rel_path):
            ...
            ungrouped_entities: List[CodeEntity] = []
            ...
            for ent in f.entities:
                ...
                if group:
                    ...
                    ungrouped_entities.append(ent)

            if not ungrouped_entities and not grouped_funcs:
                continue

            # Check if any entity in this file is hot
            file_has_hot = any(e.name in log_functions for e in ungrouped_entities)
            file_marker = " [ACTIVE]" if (HIGHLIGHT_FROM_LOG and file_has_hot) else ""
            if file_has_hot:
                hot_file_count += 1

            # Check for task relevance - only show if score is above threshold
            task_marker = ""
            if f.task_relevance_score >= TASK_RELEVANCE_THRESHOLD and f.task_keyword_matches:
                # Only show high-value matches in the tag
            ...
            # Output grouped functions first
            ...
            # Output ungrouped entities
            entities = sorted(ungrouped_entities, key=lambda x: x.line_start)
            for ent in entities:
                loc = ent.line_end - ent.line_start
                unique_calls = sorted(list(set(ent.resolved_calls)))
                
                # Build markers
                markers = []
                
                # Tauri command marker
                if ent.is_tauri_command:
                    markers.append("[CMD]")
                ...
                if HIGHLIGHT_FROM_LOG and is_hot:
                    ...
                    if call_count > 5:
                        markers.append("[HOT x{}]".format(call_count))
                    elif call_count > 1:
                        markers.append("[x{}]".format(call_count))
                    else:
                ...
                # --- SKIP EMPTY STRUCTS ---
                if ent.kind in ["struct", "enum", "trait"] and loc == 0:
                    map_lines.append("   - **{}** `{}`".format(ent.name, ent.kind))
                    continue

                # --- TRIVIAL COMPRESSION ---
                is_trivial = (
                    ent.kind not in ["class", "struct", "enum", "trait"] and
                    (loc < TRIVIAL_LOC_THRESHOLD or is_trivial_by_name(ent.name)) and
                    len(unique_calls) <= MAX_CALLS_FOR_TRIVIAL
                )

                if is_trivial:
                    call_summary = ""
                    if len(unique_calls) == 1:
                        call_summary = " --> {}".format(unique_calls[0])
                    elif len(unique_calls) > 1:
                        call_summary = " --> [Multiple]"
                    map_lines.append("   - **{}** ({} loc){}{}".format(
                        ent.name, loc, marker_str, call_summary))
                    continue

                # --- DETAILED FORMAT ---
                map_lines.append("   - **{}** `{}` ({} loc){}".format(
                    ent.name, ent.kind, loc, marker_str))

                if ent.kind not in ["struct", "class", "enum", "trait"]:
                    sig = ent.signature.replace("\n", " ")
                    if len(sig) > 100:
                        sig = sig[:97] + "..."
                    map_lines.append("     Sig: {}".format(sig))

                # Show hook dependencies if present (comma-separated, not chained)
                if ent.hook_dependencies:
                    map_lines.append("     Hooks: {}".format(", ".join(ent.hook_dependencies)))

                if unique_calls:
                    for c in unique_calls:
                        called_name = c.split("::")[-1].strip()
                        call_hot = " [*]" if (HIGHLIGHT_FROM_LOG and called_name in log_functions) else ""
                        map_lines.append("       --> {}{}".format(c, call_hot))

    # Add log analysis summary at the end
    if HIGHLIGHT_FROM_LOG and log_functions:
        map_lines.append("\n## LOG ANALYSIS SUMMARY")
        map_lines.append("- Functions detected in log: {}".format(len(log_functions)))
        map_lines.append("- Hot files (contain called functions): {}".format(hot_file_count))
