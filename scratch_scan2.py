...
for dp,_,fs in os.walk(root):
    for f in fs:
        ...
        for m in pat_fn.finditer(src):
            ...
            if g and gs and g.start()<gs.start():
                res.append((p.replace(os.sep,'/'), m.group(3)))
for v in sorted(set(res)): print(v[0], v[1])
print(len(set(res)),'functions take grid before grids')
