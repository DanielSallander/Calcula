...
//! PURPOSE: Local filesystem workspace adapter.
//! CONTEXT: A workspace is a directory on disk that hosts .calp applications.
//! Each application is a subdirectory containing calp-manifest.json and
...
// ---------------------------------------------------------------------------
// D7 — workspace robustness primitives
// ---------------------------------------------------------------------------

/// Validate a single path component (application name, version, submitter id) that
/// originates from a third-party application manifest or a spoofable identity file
/// before it is joined into a filesystem path. Rejects anything that could
/// escape the workspace root or otherwise be unsafe as a directory name:
...
pub fn validate_component(component: &str, kind: &str) -> Result<(), CalpError> {
    let invalid = |reason: &str| {
        Err(CalpError::Workspace(format!(
...
fn atomic_write(path: &Path, content: &[u8]) -> Result<(), CalpError> {
    let parent = path
        .parent()
        .ok_or_else(|| CalpError::Workspace(format!("path has no parent: {}", path.display())))?;
    fs::create_dir_all(parent)?;
    let file_name = path
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| CalpError::Workspace(format!("path has no file name: {}", path.display())))?;
...
/// A best-effort cross-process advisory lock over a workspace. Held across the
/// whole workspace-write phase of a publish — the gate checks (does this version
...
/// and stolen, so a crash can never deadlock the workspace forever.
///
            ...
            match fs::OpenOptions::new().write(true).create_new(true).open(&path) {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                    ...
                    if start.elapsed() > max_wait {
                        return Err(CalpError::WorkspaceBusy {
        ...
        let handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                while !beat_stop.load(Ordering::Relaxed) {
                    ...
                    since_touch += tick;
                    if since_touch >= WorkspaceLock::HEARTBEAT {
            ...
            .ok();
        WorkspaceLock { path, stop, heartbeat: handle }
    }
}
...
impl LocalWorkspace {
    ...
    /// at in a picker than a folder, so the UI offers the marker. Both forms
    ...
    /// Write the `workspace.calcula` marker if it is not already there.
    ///
    /// Called from the WRITE paths only (publishing into a workspace, and adding
    /// one deliberately). `open` must not write: it runs on read paths too, and
    /// a workspace on a read-only share has to stay browsable.
    pub fn ensure_marker(&self) -> Result<(), CalpError> {
        let path = self.root.join(crate::workspace_id::WORKSPACE_MARKER_FILE);
        if path.exists() {
            return Ok(());
        }
        // Deliberately minimal. The marker exists to be POINTED AT and to say
        // what this folder is; the applications beside it are the content, and
        // anything duplicated in here would be a second source of truth.
        let body = serde_json::json!({
            "formatVersion": 1,
            "kind": "calcula-workspace",
        });
        atomic_write(&path, serde_json::to_string_pretty(&body)?.as_bytes())
    }
    ...
    pub fn get_application_manifest(&self, package_name: &str) -> Result<ApplicationManifest, CalpError> {
        let path = self.application_dir(package_name)?.join("calp-manifest.json");
        if !path.exists() {
            return Err(CalpError::ApplicationNotFound(package_name.to_string()));
        }
        let content = fs::read_to_string(&path)?;
        let manifest: ApplicationManifest = serde_json::from_str(&content)?;
        Ok(manifest)
    }

    /// Write an application manifest (atomically — D7).
    ///
    /// PLACING AN APPLICATION IS WHAT MAKES A DIRECTORY A WORKSPACE, so this is
    /// where the `workspace.calcula` pointer file gets written. It lives here,
    /// and not in the publish commands, because there are FOUR ways to put an
    /// application into a workspace — `publish::publish`, `skin_pack::skin_publish`,
    /// the model publish and the library publish — and every one of them writes
    /// this manifest. The call used to sit in `calp_publish` alone: publishing a
    /// MODEL or a skin pack into a fresh folder therefore produced a workspace
    /// full of applications with no pointer file, which the Subscribe and
    /// Open-for-editing pickers (file-only, by design) could not see at all.
    /// A rule enforced at three of five call sites is not enforced.
    ///
    /// Best-effort: a workspace we are already writing artifacts into can
    /// virtually always take one more small file, and a pointer that could not
    /// be written must never fail a publish that otherwise succeeded.
    pub fn write_application_manifest(&self, manifest: &ApplicationManifest) -> Result<(), CalpError> {
        let _ = self.ensure_marker();
        let path = self.application_dir(&manifest.name)?.join("calp-manifest.json");
        let content = serde_json::to_string_pretty(manifest)?;
        atomic_write(&path, content.as_bytes())
    }

    /// Acquire the workspace's cross-process advisory lock (D7). Hold it across
    /// an application-manifest read-modify-write (get_application_manifest -> mutate ->
    ...
    /// List all available versions for an application (as SemVer, sorted).
    ...
    pub fn save_submission(
        ...
        // anything that could escape the submissions directory. (Application /
    ...
    /// Load the RAW submission events for an application version across all
    ...
    /// Load the RAW review events for an application version (hygiene filtered:
    ...
    /// Load the CURRENT submissions for an application version: the raw events
    ...
    // -----------------------------------------------------------------------
    // Artifact storage (D8 — WorkspaceTransport surface)
    // -----------------------------------------------------------------------
    ...
    /// validated at the workspace boundary (D7) via `version_dir`. The rel_path
    ...
    /// Path of a content-addressed blob. Blobs are shared across ALL applications and
    ...
    fn blob_path(&self, hash: &str) -> Result<PathBuf, CalpError> {
        if hash.len() < 4 || !hash.bytes().all(|b| b.is_ascii_hexdigit()) {
            return Err(CalpError::Workspace(format!("invalid blob hash '{hash}'")));
        }
    ...
    fn application_dir(&self, package_name: &str) -> Result<PathBuf, CalpError> {
        validate_component(package_name, "application name")?;
        Ok(self.root.join(package_name))
    }

    /// Get the directory path for a specific application version.
    /// Public so publish/pull can write and verify artifacts in place.
    /// Validates `package_name` + `version` at the workspace boundary (D7).
    pub fn version_dir(&self, package_name: &str, version: &str) -> Result<PathBuf, CalpError> {
        validate_component(version, "version")?;
        Ok(self.application_dir(package_name)?.join(version))
    }
...
// ---------------------------------------------------------------------------
// D8 — LocalWorkspace is one WorkspaceTransport implementation behind the seam.
...
// touching publish/pull/integrity, which operate on `&dyn WorkspaceTransport`.
// ---------------------------------------------------------------------------
impl WorkspaceTransport for LocalWorkspace {
    fn list_applications(&self) -> Result<Vec<String>, CalpError> {
        LocalWorkspace::list_applications(self)
    }
    ...
    fn read_application_artifact(
        ...
        validate_component(rel_path, "application artifact name")?;
        let path = self.application_dir(package_name)?.join(rel_path);
        if !path.exists() {
            return Ok(None);
        }
    ...
    fn get_application_manifest(&self, package_name: &str) -> Result<ApplicationManifest, CalpError> {
        LocalWorkspace::get_application_manifest(self, package_name)
    }

    fn write_application_manifest(&self, manifest: &ApplicationManifest) -> Result<(), CalpError> {
        LocalWorkspace::write_application_manifest(self, manifest)
    }
    ...
    fn get_version_manifest(
        ...
        LocalWorkspace::get_version_manifest(self, package_name, version)
    }
    ...
    fn write_version_manifest(
        ...
        LocalWorkspace::write_version_manifest(self, package_name, version, manifest)
    }

    fn version_exists(&self, package_name: &str, version: &str) -> bool {
        LocalWorkspace::version_exists(self, package_name, version)
    }
    ...
    fn resolve_version(
        ...
        LocalWorkspace::resolve_version(self, package_name, pin)
    }

    fn list_versions(&self, package_name: &str) -> Result<Vec<SemVer>, CalpError> {
        LocalWorkspace::list_versions(self, package_name)
    }
    ...
    fn write_artifact(
        ...
        LocalWorkspace::write_artifact(self, package_name, version, rel_path, bytes)
    }
    ...
    fn read_artifact(
        ...
        LocalWorkspace::read_artifact(self, package_name, version, rel_path)
    }
    ...
    fn list_artifacts(
        ...
        LocalWorkspace::list_artifacts(self, package_name, version)
    }

    fn clear_version(&self, package_name: &str, version: &str) -> Result<(), CalpError> {
        LocalWorkspace::clear_version(self, package_name, version)
    }
    ...
    fn commit_artifacts_as_blobs(
        ...
        LocalWorkspace::commit_artifacts_as_blobs(self, package_name, version, checksums)
    }
    ...
    fn save_submission(
        ...
        LocalWorkspace::save_submission(self, package_name, version, submission)
    }
    ...
    fn save_review(
        ...
        LocalWorkspace::save_review(self, package_name, version, review)
    }
    ...
    fn load_review_events(
        ...
        LocalWorkspace::load_review_events(self, package_name, version)
    }
    ...
    fn load_current_submissions_by(
        ...
        LocalWorkspace::load_current_submissions_by(self, package_name, version, submitter_id)
    }
    ...
    fn load_current_region_submissions(
        ...
        LocalWorkspace::load_current_region_submissions(self, package_name, version, region_id)
    }
    ...
    fn load_current_submissions(
        ...
        LocalWorkspace::load_current_submissions(self, package_name, version)
    }
...
mod tests {
    ...
    fn create_test_registry() -> (TempDir, LocalWorkspace) {
        let dir = TempDir::new().unwrap();
        let reg = LocalWorkspace::open(dir.path()).unwrap();
        (dir, reg)
    }

    fn create_test_package(reg: &LocalWorkspace, name: &str) -> ApplicationManifest {
        let mut manifest = ApplicationManifest::new(name, "report", "test-author", "2026-01-01T00:00:00Z");
        manifest.description = format!("Test application: {}", name);
        reg.write_application_manifest(&manifest).unwrap();
        manifest
    }
    ...
    fn list_empty_registry() {
        ...
        assert_eq!(reg.list_applications().unwrap(), Vec::<String>::new());
    }
    ...
    fn create_and_list_packages() {
        ...
        let packages = reg.list_applications().unwrap();
    ...
    fn read_package_manifest() {
        ...
        let manifest = reg.get_application_manifest("my-package").unwrap();
    ...
    fn package_not_found() {
        let (_dir, reg) = create_test_registry();
        let result = reg.get_application_manifest("nonexistent");
        assert!(matches!(result, Err(CalpError::ApplicationNotFound(_))));
    }
    ...
    fn path_methods_reject_hostile_package_version_and_submitter() {
        let (_dir, reg) = create_test_registry();
        assert!(reg.get_application_manifest("../escape").is_err());
    ...
    fn transport_artifact_roundtrip_and_list_exclusions() {
        let (_dir, reg) = create_test_registry();
        // Drive the workspace purely through the trait object.
        let t: &dyn WorkspaceTransport = &reg;
