...
pub use parser::ast::{
    Expression, Value, BinaryOperator, UnaryOperator, BuiltinFunction, TableSpecifier,
};

// (mirrored enums removed — now re-exported from parser above)
//
// NOTE: If you see a compile error about a missing variant, you only need to
// add it to the parser's BuiltinFunction enum in core/parser/src/ast.rs.
// The engine picks it up automatically via the re-export.
